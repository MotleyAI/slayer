...
**Aggregation caveats:**
- **SQLite**: `median`, `percentile_cont`, `percentile_disc`, `stddev_samp`, `stddev_pop`, `var_samp` (also aliased as `variance`), `var_pop` (also aliased as `variance_pop`), `corr`, `covar_samp`, `covar_pop` are provided via Python aggregate UDFs registered on every new connection (`slayer/sql/sqlite_udfs.py`); SQLite has no native equivalent. Scalar UDFs `ln`, `log10`, `exp`, `sqrt`, `pow`, `power` are also registered there. The 2-arg `log(B, X)` UDF (returns log_B(X) — base first, value second) is registered only on SQLite < 3.35; newer versions ship it as a built-in with the same arg order.

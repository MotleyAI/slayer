...
- Models can have explicit `joins` to other models (LEFT JOINs). Cross-model measures use dotted syntax (`customers.avg_score`) and generate sub-queries for correct aggregation
- `SlayerQuery.model` accepts a model name or a nested `SlayerQuery` (query-as-model). `create_model_from_query()` saves a query as a permanent model

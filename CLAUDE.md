...
- **Schema versioning**: `SlayerModel`, `SlayerQuery`, `NamedQuery`, and `DatasourceConfig` carry a `version: int` (currently `1`). On load, older versions are upgraded via the converter chain in `slayer/storage/migrations.py` before Pydantic validates the dict. Saves always emit the current version. The hook is on the Pydantic class itself (`@model_validator(mode="before")`), so every storage backend — YAML, SQLite, third-party backends, plus MCP/API/dbt entry points — gets migrations automatically without backend changes. See [docs/concepts/models.md](docs/concepts/models.md#schema-versioning).
- **Named queries (S2)**: `NamedQuery` is a stored multistage query — a `name`, optional `description`, a list of `stages: List[SlayerQuery]`, and a top-level `variables: Dict[str, Any]`. Stage *N+1* may reference stage *N*'s `name` via `source_model`; the final stage may be anonymous. Saved at `<storage>/queries/<name>.yaml` (YAML) or in a `queries` table (SQLite). `engine.execute(query="<name>")` loads the query and runs the stages with merged variables (precedence: `stage.variables` > runtime > `NamedQuery.variables`). `save_query` validates via dry-run with placeholder `0` for unsupplied variables. Bidirectional name-collision check between `SlayerModel.name` and `NamedQuery.name` lives in the `StorageBackend` ABC's concrete `save_*` methods (single namespace, single rule, all backends inherit). MCP tools: `list_queries`, `inspect_query`, `run_named_query`, `save_query`, `delete_query`. CLI: `slayer queries {list,show,save,delete,run,inspect}`. HTTP: `/queries` REST endpoints. See [docs/examples/09_named_queries](docs/examples/09_named_queries/named_queries.md).
...
- **Engine is async-first**: `SlayerQueryEngine.execute()` is `async`. Use `execute_sync()` for CLI/notebooks/scripts. Accepts a `SlayerQuery`, a list of stages, a dict, a `NamedQuery`, **or a string** (a stored NamedQuery name — engine loads + runs).
...
- `slayer queries {list,show,save,delete,run,inspect}` manages stored named queries (multistage). `run --variables k=v,k=v` supplies runtime values for `{var}` placeholders; `inspect` returns the saved stages plus the final-stage column schema (run via dry-run probe).

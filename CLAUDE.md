...
- Column SQL uses bare column names for columns from the model's own underlying sql/table (e.g., `"amount"`). A derived `Column.sql` may reference any other derived column — local same-model or via the join graph (single-dot `B.col` or `__`-delimited `B__C.col` path) — and the engine recursively inlines those references at query time so chains like `A.ratio_using_derived = "A.bar / B.foo_normalized"` (where `B.foo_normalized` is itself derived) work without manual inlining. When a query-, model-, or column-level filter references a bare-named local derived column whose `sql` crosses a join, the planner walks the chain and adds the implied LEFT JOINs automatically — the column does not need to also appear in `dimensions`.
- **Unresolvable filter / dim / measure refs raise at translate time** (DEV-1367). Any filter (query/model/`Column.filter`), dimension, time-dimension, or cross-model measure that names a model unreachable via the source's join graph — or a column/measure that doesn't exist on the resolved terminal model — raises `ValueError` from `enrich_query` with a filter-aware message. Replaces the prior silent `break` in `_resolve_joins` that produced unbound `WHERE`-clause references and cryptic database-runtime errors. Two layers: F2 validates each filter string up-front (`_validate_filter_string` in `slayer/engine/enrichment.py`) with a filter-aware error and a leaf-column existence check; R2 raises in `_resolve_joins` (`enrichment.py`) on any unresolvable path tuple, with origin provenance so the message names the dimension / filter / cross-model measure that introduced the bad path. Re-rooted CTEs (`_build_rerooted_enriched`) call `_enrich(strict_joins=False)` because they intentionally drop unreachable refs from the rebuilt query. The bare-name allowlist (model columns ∪ saved measures ∪ custom-agg names ∪ SQL keywords ∪ canonical agg aliases like `_count`, `revenue_sum`) keeps `"x is null"`, `"y = true"`, and post-aggregation HAVING-style filters from tripping the guard.
...
- **Window functions in filters**: filter strings and `ModelMeasure.formula` cannot contain raw `OVER (...)` SQL — SLayer's formula parser is Python-AST-based and rejects with an actionable error pointing at the `rank()` / `first()` / `last()` / `lag()` / `lead()` transforms or a `Column.sql`-with-window pattern. A filter on a `Column` whose `sql` does contain a window function is auto-promoted: SLayer materializes the column in the base CTE under its alias, then applies the predicate as a post-aggregation outer `WHERE` (no manual subquery / multi-stage model needed). Use `Column(name="rn", sql="row_number() over (order by mass desc)", type=DataType.INT)` + `filters=["rn <= 3"]` for top-N, or the `rank(<measure>) <= N` transform inline.
...
- **Memories** (DEV-1357): An agent-memory layer indexed by canonical entity strings. Three tools — `save_memory(learning, linked_entities)`, `forget_memory(id)`, `recall_memories(about, max_learnings, max_queries)` — let agents record per-entity notes (and optionally an example `SlayerQuery`), then look them up before drafting a new query. `linked_entities` and `about` accept either a list of entity strings (resolved strictly) or an inline `SlayerQuery`/dict (entities auto-extracted; warnings non-fatal; the query is also persisted on the memory in the save path). The canonical form is exactly one of `<ds>`, `<ds>.<model>`, `<ds>.<model>.<leaf>` (≤ 3 dotted segments after canonicalisation). Aggregation suffixes are stripped (`revenue:sum` → `<ds>.<model>.revenue`); `*:count` collapses to the source model; multi-hop dotted paths keep only the leaf (`orders.customers.regions.name` → `{<orders.ds>.orders, <regions.ds>.regions.name}`). The resolver lives in `slayer/memories/resolver.py`; the unified `Memory` row + storage primitives are concrete on `StorageBackend` (ID format / monotonic non-reuse / entity-intersection filter), with backends only implementing the row-shaped CRUD + a single `memory_seq` counter. `inspect_model` auto-renders a `Learnings` section listing only memories where `query is None`; query-bearing memories surface only via `recall_memories`. Memory ids are monotonic positive ints and never reused. The same surface is exposed across MCP, REST (`POST /memories`, `DELETE /memories/{id}`, `POST /memories/recall`), CLI (`slayer memory {save,forget,recall}`), and `SlayerClient`. See [docs/concepts/memories.md](docs/concepts/memories.md).

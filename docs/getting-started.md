...
# Specific extras
pip install semantic-slayer[api]      # REST API (FastAPI + uvicorn)
...
Once connected, the agent will call `create_datasource` (which auto-ingests models by default) then `models_summary` then `query` conversationally. Set `auto_ingest=false` to skip auto-ingestion and call `ingest_datasource_models` separately.

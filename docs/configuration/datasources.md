...
| Type | Connection Driver | Example |
|------|-------------------|---------|
| `postgres` / `postgresql` | `postgresql://` | `postgresql://user:pass@localhost:5432/db` |
| `mysql` / `mariadb` | `mysql://` | `mysql://user:pass@localhost:3306/db` |
| `clickhouse` | `clickhouse+http://` | `clickhouse+http://user:pass@localhost:8123/db` |
| `sqlite` | `sqlite:///` | `sqlite:///path/to/db.sqlite` |
| `bigquery` | BigQuery adapter | Via connection string |
| `snowflake` | Snowflake adapter | Via connection string |

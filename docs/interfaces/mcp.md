...
| Tool | Description |
|------|-------------|
| `models_summary` | List all models with schemas (dimensions, measures). Returns JSON. |
...
| `fields` | list[dict] | Data columns: measures, arithmetic, transforms. E.g. `[{"formula": "count"}, {"formula": "revenue / count", "name": "aov"}, {"formula": "cumsum(revenue)"}]`. Supports nesting: `{"formula": "change(cumsum(revenue))"}` |
| `dimensions` | list[str] | Dimension names, e.g. `["status"]` |
| `filters` | list[str] | Filter formula strings, e.g. `["status == 'active'", "amount > 100"]`. Supports operators (`==`, `!=`, `>`, `>=`, `<`, `<=`, `in`, `is None`, `is not None`), boolean logic (`and`, `or`, `not`), and functions (`contains`, `starts_with`, `ends_with`, `between`, `having`). |
| `time_dimensions` | list[dict] | Time grouping |
| `order` | list[dict] | Sorting, e.g. `[{"column": "count", "direction": "desc"}]` |
| `limit` | int | Max rows |
| `offset` | int | Skip rows |
| `variables` | dict | Template variables for filter placeholders |
...
| `add_measures` | Add measures incrementally. Params: `model_name`, `measures` list. |
| `add_dimensions` | Add dimensions incrementally. Params: `model_name`, `dimensions` list. |
| `delete_measures_dimensions` | Remove measures or dimensions by name. Params: `model_name`, `names` list. |
| `update_model` | Update model metadata (description, data source). Params: `model_name`, `description`, `data_source`. |
...
2. models_summary()                            # see what was generated
...
5. models_summary()                            # see what was generated
```

### Query data

```
1. models_summary()                            # discover models
...
```
1. add_measures(model_name="orders", measures=[{"name": "avg_amount", "sql": "amount", "type": "avg"}])
2. add_dimensions(model_name="orders", dimensions=[{"name": "priority", "sql": "priority", "type": "string"}])
3. delete_measures_dimensions(model_name="orders", names=["amount_sum"])
```

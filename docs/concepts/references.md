...
* `{variable}` placeholders are substituted into these Mode-A surfaces from the merged variable set (raise-on-missing, Mode-A-aware string escaping). See [Variables in model SQL](models.md#variables-in-model-sql).

...
The `name` is optional — if omitted, it's derived by sanitizing the formula into a bare identifier (`"revenue:sum / *:count"` → `revenue_sum_count`; very long names hash-fold). The `label` is an optional human-readable display name for the field.

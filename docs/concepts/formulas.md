...
The 2-arg `log(B, X)` UDF is registered only on SQLite < 3.35; newer versions ship the built-in (with the same B-first arg order) and we use that. `ln`, `log10`, and `log2` always register; the `log2` UDF overrides SQLite ≥3.35's silent-NULL built-in to keep the strict math-domain-error semantics.

...
`greatest` / `least` / `trunc` / `mod` were the last four parser-only names
(DEV-1753): admitted to the binder to match the parser's `SCALAR_PASSTHROUGH`,
...
SQLite/MySQL/BigQuery — a ratified divergence, see `DECISIONS.md`).

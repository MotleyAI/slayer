...
The one place you *don't* write quotes is an `IN`-list — `region IN ({regions})`
takes a list and quotes each element for you (a bare string there would land
unquoted). Models generated by `import-cube` normalize a scalar to a
one-element list for those pushdowns.

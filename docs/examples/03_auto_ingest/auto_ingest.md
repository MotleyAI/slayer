...
| Numeric, non-ID | One measure per column (e.g., `{name: "amount", sql: "amount"}`). Aggregate at query time: `sum(amount)`, `avg(amount)`, etc. |
| Non-numeric, non-ID | One measure per column. Use `count_distinct(name)` at query time. |

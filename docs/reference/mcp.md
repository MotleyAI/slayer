...
| `inspect_model` | Complete view of a single model: metadata with row count, any model-level or measure-level filters, dimensions table (with a `sampled` column — distinct values for string/boolean dims, `min .. max` for number/date/time dims), measures table (with `allowed_aggregations`, `filter`, `label`, `description`, `sql`), custom aggregations, joins (direct/multi-hop), all fields reachable via joins up to depth 5, and a sample-data table. Every markdown table auto-prunes all-empty columns and collapses to a comma-separated backticked list when only one column remains. Params: `model_name`, `num_rows` (default 3), `show_sql` (default false — include SQL for the sample-data query), `format` (default `"markdown"`; also `"json"`). |

...
| `strict` | bool | Default `false` — error instead of warn when a [cross-model measure broadcasts](../concepts/queries.md#cross-model-measures) or a filter is dropped from its producer. Rejected with the run-by-name shortcut — declare it on the stored query instead |

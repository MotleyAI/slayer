...
| `create_model` | Create a model from a table/SQL definition or from a query. Pass `sql_table`/`sql` with `columns` (and optional named-formula `measures` and custom `aggregations`) for table-based, or pass `query` (a SLayer query dict) to save it as a query-backed model whose `columns` + `backing_query_sql` are populated by a save-time dry-run. |
...
1. edit_model(
     ...
     measures=[{"name": "aov", "formula": "sum(revenue) / count(*)", "label": "Average Order Value"}],

...
def _outer_order_terms(sql: str, dialect: str = "postgres") -> list[tuple[str, str]]:
    """Return each ORDER BY term from the OUTERMOST SELECT as ``(expression_sql, direction)`` pairs where direction is ``"asc"`` or ``"desc"``. Used by tests that assert two ORDER BY terms aren't byte-identical AND that the direction wasn't lost in the outer wrap."""
...
def _projection_rn_by_alias(sql: str, dialect: str = "postgres") -> dict[str, str]:  # NOSONAR(S3776) — sequential isinstance dispatch over outermost-SELECT projection layers (Alias / Cast / Max / Case / EQ / Column unwrap). Each layer is a structural-shape predicate whose failure short-circuits to the next projection; extracting per-layer helpers would scatter the predicate.
    """For the OUTERMOST SELECT, walk projections and return ``{alias: rn_column_name}`` for every projection whose body is a ``MAX(CASE WHEN <_first_rn|_last_rn[suffix]> = 1 THEN …)`` aggregate (wrapped in optional ``CAST(...)``). The alias is the ``AS …`` body; the rn column is the identifier referenced in the CASE WHEN."""
    tree = sqlglot.parse_one(sql, dialect=dialect)
    if not isinstance(tree, sqlglot.exp.Select):
        return {}
    out: dict[str, str] = {}
...
def _outer_from_node(sql: str, dialect: str = "postgres"):
    """Return the OUTERMOST SELECT's FROM source node (a sqlglot ``Table`` for a flat SELECT, a ``Subquery`` for an outer-wrap shape). sqlglot uses ``from_`` as the arg key and stores the single source at ``.this``, not in ``.expressions`` (which is empty for a single FROM)."""
    tree = sqlglot.parse_one(sql, dialect=dialect)
    if not isinstance(tree, sqlglot.exp.Select):
        return None
...
async def _generate(
    ...
    *,
    ...
    """Run ``query`` against ``model`` through the typed engine pipeline and return the emitted SQL."""
...
def generator() -> SQLGenerator:
    # Mostly a per-test dialect carrier; a few tests call gen._build_agg / _build_percentile / _build_stat_agg directly with an AggRenderSpec.
    return SQLGenerator(dialect="postgres")


class TestBasicQueries:
    async def test_numeric_literal_measure(self, generator: SQLGenerator) -> None:
        """Measures with numeric SQL expressions (e.g. dbt `expr: 1`) should generate SUM(1), not SUM(model."1")."""
        model = SlayerModel(
            name="policy",
            sql_table="policy",
            data_source="test",
        ...
        # Literal Column.sql "1" wraps in CAST when type is set (SUM(CAST(1 AS ...))); it must never be quoted as identifier "1".
    ...
    async def test_order_by_ascending_synonym_not_descending(
        ...
        # DEV-1575 regression: a canonical "ASCENDING" must normalize to asc so the generator's strict ``direction == "asc"`` check does NOT fall through to DESC.
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
            dimensions=[ColumnRef(name="status")],
...
class TestFilters:
    ...
    async def test_contains_filter(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        # Mode-B query filters use the ``like(value, pattern)`` scalar form; it emits the SQL ``LIKE`` operator. (Native ``x LIKE y`` operator syntax remains available in Mode-A model-level filters.)
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
    ...
    async def test_like_wrong_arity_raises(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        # ``like`` requires exactly (value, pattern); a single arg is rejected at bind time rather than emitting broken SQL.
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
            filters=["like(status)"],
...
class TestFields:
    async def test_arithmetic_field(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        """Arithmetic over aggregates is emitted inline in the SELECT (the typed pipeline folds ``revenue:sum / *:count`` into a single grouped SELECT — no CTE needed for simple aggregate arithmetic)."""
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
    ...
    async def test_field_with_limit(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        """LIMIT applies to the final SELECT. The typed pipeline folds the aggregate arithmetic into one grouped SELECT, so LIMIT trails the single FROM clause."""
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count"), ModelMeasure(formula="revenue:sum"), ModelMeasure(formula="revenue:sum / *:count", name="aov")],
            limit=5,
    ...
    async def test_cumsum_over_week_sunday_time_dim(
        ...
        """DEV-1572 (full enum wiring): a transform over a WEEK_SUNDAY time dimension must generate valid SQL — the granularity must be wired into the interval helpers, not crash with a KeyError on the 9th enum value."""
        orders_model.default_time_dimension = "created_at"
        query = SlayerQuery(
            source_model="orders",
        ...
        assert "OVER" in sql
        assert "rev_running" in sql.lower()
        # The WEEK_SUNDAY time dim must compile to the Sunday-week truncation (the +1-day column-side shift is unique to WEEK_SUNDAY; plain Monday WEEK has no such shift), proving the granularity wasn't downgraded.
    ...
    async def test_time_shift_over_week_sunday_uses_one_week_interval(
        ...
        # The shift must use a one-WEEK interval; target INTERVAL '1 WEEK' so the DATE_TRUNC('WEEK', INTERVAL '1 DAY') can't satisfy it.
    ...
    async def test_consecutive_periods_uses_reset_group_ctes(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        ...
        assert 'PARTITION BY "orders.status"' in norm
        assert 'ORDER BY "orders.created_at"' in norm
        # Value CTE: partition adds the reset-group alias (the typed pipeline uses a dotted ``orders.positive_streak`` separator, not ``__``).
    ...
    async def test_consecutive_periods_no_implicit_nulls_last_sqlite(
        ...
        """Regression: sqlglot's `exp.Ordered` injects `NULLS LAST` on SQLite even when not requested, which would change consecutive_periods streak/reset semantics for any NULL time values vs. the pre-AST string-built `ORDER BY <t>` output. The fix is to put a bare column inside `exp.Order` rather than wrapping it in `exp.Ordered`."""
    ...
    async def test_windowed_sum_uses_range_join_primitive(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        ...
        sql = await _generate(generator=generator, query=query, model=orders_model)
        norm = _norm(sql)
        # DEV-1835: windowed measures now emit the unified ``_cm_`` producer CTE named after the canonical aggregate alias (``revenue_sum_window_90d``), while the surfaced output column keeps the user ``name`` (revenue_90d).
        assert "_cm_orders__revenue_sum_window_90d" in norm
        assert "LEFT JOIN" in norm
        assert "_src._w_time >=" in norm
        assert "_src._w_time <" in norm
        # AST-based generation renders single-unit intervals via sqlglot's per-dialect transpiler — Postgres caps the unit name.
        assert "INTERVAL '90 DAY'" in norm
        # The inner grain comparison is NULL-SAFE: a group whose dimension is NULL must receive its real windowed value, not NULL. The time-range bounds above stay plain inequalities — they are a range, not grain.
        assert (
    ...
    async def test_windowed_sum_preserves_other_time_dim_grain(
        ...
        """With 2+ time dimensions, the windowed CTE must equality-join on every non-window time dim — otherwise rows from other dim values fan in."""
        orders_model.default_time_dimension = "created_at"
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[
    ...
    async def test_windowed_sum_keeps_joins_used_by_query_filter(
        ...
        """Window CTE must keep joins whose alias is referenced by a query-level WHERE filter, even if the windowed measure itself doesn't use them."""
        engine, orders = orders_with_customers_engine
        # Filter on customers.region_id forces a customers join. The windowed measure does not otherwise reference customers, so the join would be pruned without the filter-aware logic — and then the WHERE clause below would reference an undefined alias.
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
    ...
    async def test_windowed_sum_keeps_transitive_joins_for_multi_hop_filter(
        ...
        """Multi-hop filter (e.g. customers.regions.name) must keep the intermediate `customers` join in the _src subquery."""
        storage = YAMLStorage(base_dir=str(tmp_path))
        await storage.save_datasource(DatasourceConfig(name="test", type="postgres"))
        await storage.save_model(SlayerModel(
    ...
    async def test_filter_on_windowed_measure_is_post_filter(
        ...
        """A filter on a windowed measure must apply post-aggregation, not as HAVING on the base CTE. The base CTE doesn't compute the windowed value — applying a HAVING there would use the wrong (non-windowed) aggregate."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
            measures=[{"formula": "revenue:sum(window='90d')", "name": "revenue_90d"}],
        ...
        sql = await _generate(generator=generator, query=query, model=orders_model)
        norm = _norm(sql)
        # DEV-1443: the colon-syntax filter resolves to the renamed user alias. CodeRabbit nitpick: target the post-filter WHERE clause specifically — the alias surfaces in the projection too, so a generic ``in norm`` check could pass even on regression.
        ...
        # The filter must be applied OUTSIDE the base CTE (no HAVING on the plain `SUM(amount)` aggregate — that would use the wrong value).
    ...
    async def test_window_duration_full_compact_syntax(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        ...
        # One INTERVAL per (amount, unit) pair, chained as subtractions; sqlglot transpiles each per dialect (correct on MySQL/ClickHouse/BigQuery).
        for piece in (
            "INTERVAL '1 YEAR'",
            "INTERVAL '2 MONTH'",
            "INTERVAL '3 WEEK'",
    ...
    async def test_two_windowed_measures_emit_distinct_ctes(
        ...
        """Two windowed measures in one query must each get their own collision-safe ``_cm_`` CTE and both join back to the base grain."""
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
        ...
        # Codex round 3: deterministic order — CTEs follow measure declaration order (rev_90d before rev_30d), never set-iteration order.
    ...
    async def test_windowed_source_column_filter_wraps_case_in_src(
        ...
        """A windowed measure whose source ``Column`` carries a ``filter=`` must CASE-wrap the value inside ``_src`` (``_w_value``), mirroring the legacy filtered-aggregate shape."""
        model = SlayerModel(
    ...
    async def test_windowed_source_column_filter_crossing_join_kept_in_src(
        ...
        """DEV-1714 (Codex C3): a windowed measure whose source ``Column.filter`` crosses a join must pull that join INTO ``_src`` (predicate-carrier join discovery via the resolver's filter path, not just a local CASE)."""
        ...
        src_body = _extract_src_body(sql)
        # Assert a real JOIN was registered — not merely that the alias appears inside the CASE predicate text (which would pass even if discovery were broken).
    ...
    async def test_windowed_model_filter_crossing_join_kept_in_src(
        ...
        """DEV-1714 (Codex C3): a model-level ``SlayerModel.filters`` entry that crosses a join must pull that join INTO ``_src`` (the Mode-A filter-text carrier path, distinct from the query-filter path already pinned)."""
        customers = SlayerModel(
            name="customers", sql_table="customers", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        assert "_cm_" in sql, sql
        src_body = _extract_src_body(sql)
        # Assert a real JOIN was registered (walk the JOIN nodes), not just an alias appearing in the filter predicate text.
        assert "customers" in _join_aliases(src_body), (
    ...
    async def test_order_by_windowed_alias_resolves(
        ...
        """ORDER BY the windowed measure's public alias must resolve against the combined SELECT (the ``_cm_`` join-back column), not the base."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
            measures=[{"formula": "revenue:sum(window='90d')", "name": "revenue_90d"}],
        ...
        # The order term is the BARE combined-SELECT output column (the _cm_ value), never a ``_base.`` qualifier — _base excludes the windowed measure, so ``_base."orders.revenue_90d"`` would dangle.
        assert '_base."orders.revenue_90d"' not in order_portion, sql
        # And the windowed measure must NOT be materialised in _base as a dead plain aggregate (Codex re-review round 2).
    ...
    async def test_windowed_row_filter_applied_inside_src(
        ...
        """F4: a query WHERE-phase row filter constrains the ``_src`` scope (host-rooted scope inherits host row filters)."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
            measures=[{"formula": "revenue:sum(window='90d')", "name": "rev_90d"}],
    ...
    async def test_windowed_date_range_not_applied_inside_src(
        ...
        """The typed ``date_range`` on the window time dim must be STRIPPED from ``_src`` — the trailing window has to reach rows before the range start — while still bounding the outer/base grain."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(
                dimension=ColumnRef(name="created_at"),
    ...
    async def test_windowed_explicit_time_filter_does_not_truncate_src(
        ...
        """DEV-1732: an EXPLICIT relational bound on the raw window time column is a FRAME bound, not a population filter — so it bounds ``_base`` and is stripped from ``_src``, exactly like the ``date_range`` spelling of the same intent."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
            measures=[{"formula": "revenue:sum(window='90d')", "name": "rev_90d"}],
    ...
    async def test_windowed_output_is_scope_closed(
        ...
        """Acceptance: the emitted ``_cm_`` statement is scope-closed — every alias referenced in a scope is bound in that scope's own FROM/JOINs. Makes the ``assert_scope_closed`` guarantee an explicit contract rather than only the autouse harness side effect."""
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
    ...
    async def test_same_windowed_measure_two_aliases_both_surface(
        ...
        """Codex#2: the same windowed formula selected under two names interns to ONE slot / ONE ``_cm_`` CTE, but BOTH public aliases must surface in the combined projection (the CTE's single aggregate column is remapped ``AS`` each alias) — the later alias must not be dropped."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
            measures=[
    ...
    async def test_windowed_joined_time_dimension_registers_join_in_src(
        ...
        """Codex#1: a windowed measure whose window time dimension lives on a JOINED model must pull that join INTO ``_src`` (registered through the scope), not reference an unbound alias."""
        customers = SlayerModel(
            name="customers", sql_table="customers", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
    ...
    async def test_change_pct(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        ...
        assert "shifted_" in sql
        assert "LEFT JOIN" in sql
        # change_pct = (curr - prev) / NULLIF(prev, 0) — the divisor is guarded against a zero prior-period value (returns NULL, not a div-by-zero).
    ...
    async def test_multiple_last_different_time_columns(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        ...
        sql = await _generate(generator, query, orders_model)
        norm = _norm(sql)
        # Two rankings, one per measure, each in its own CTE by its own time column; the old _last_rn_2 suffix only mattered under one shared scope.
    ...
    async def test_same_explicit_time_column_shared(self, generator: SQLGenerator, orders_model: SlayerModel) -> None:
        ...
        sql = await _generate(generator, query, orders_model)
        norm = _norm(sql)
        # Two aggregates over the same time column are still two CTEs; they share the ORDER BY column but differ in direction.
        assert sql.count("ROW_NUMBER()") == 2
        assert "ORDER BY orders.ordered_at DESC" in norm, sql
    ...
    async def test_multiple_time_shifts_resolve_to_distinct_aliases(
        ...
        # Both shifts are hoisted, each under its OWN alias (legacy: ``len({t.alias for t in shifts}) == len(shifts)`` over the two ``time_shift`` transforms).
        ...
        # The two measures do NOT resolve to the same expression (legacy: ``by_name["growth"] != by_name["growth_2m"]``).
    ...
    async def test_hidden_transform_name_avoids_user_measure_collision(
        ...
        # The hoisted shift's alias must not collide with a user-declared result key (the outer projection is a superset of the expression aliases).
    ...
    async def test_hidden_transform_name_avoids_dimension_collision(
        ...
        # Disjointness: the outer projection carries the declared dimensions and time dimension, none colliding with the hoisted transform alias.
        ...
        # And the measure computes against the shift, not the same-named dimension (the corruption the alias collision caused).
        growth_sql = _alias_bodies(sql, "orders.growth")[0]
...
class TestNestedFields:
    ...
    async def test_emitted_sql_has_no_agg_placeholder(
        ...
        """DEV-1341: aggregated refs nested inside non-transform calls (``nullif``) must be fully resolved — no ``__aggN__`` placeholder may leak through."""
        query = SlayerQuery(
            source_model="orders",
            measures=[
                ModelMeasure(
...
class TestMultiDialectGeneration:
    ...
    def orders_model(self) -> SlayerModel:
        model = SlayerModel(
            ...
            columns=[
                ...
                Column(name="revenue", sql="amount", type=DataType.DOUBLE),
                # Second numeric column so 2-arg stat aggregates (corr(other=...) / covar_*(other=...)) have a valid LHS+RHS pair.
    ...
    async def test_date_trunc_casts_unknown_typed_time_dim(self, dialect: str) -> None:
        """A time-dimension whose ``sql`` is a bare literal (or any expression whose live type is ``unknown``) must be wrapped in ``CAST(... AS TIMESTAMP)`` before being passed to ``DATE_TRUNC``. Postgres has multiple overloads keyed on the second argument's type and rejects ``DATE_TRUNC('month', '2025-12-01')`` with ``AmbiguousFunctionError``."""
        gen = SQLGenerator(dialect=dialect)
        model = SlayerModel(
            name="orders",
            sql_table="public.orders",
        ...
        # sqlglot transpiles ``TIMESTAMP`` → ``DATETIME`` on MySQL / BigQuery, so we don't assert the literal target-type spelling — only that the literal is wrapped in a CAST.
        ...
        # The time-dim column must not be CAST-wrapped (its DB type is known); a measure-level CAST like CAST(COUNT(*) AS INT) is fine.
    ...
    async def test_window_measure_multi_unit_interval_dialect_correct(
        ...
        """Multi-unit windows (e.g. '1y2m3d') must render as separate per-unit INTERVAL clauses on MySQL and ClickHouse — never as a single Postgres-shaped quoted multi-unit literal which neither dialect parses."""
        gen = SQLGenerator(dialect=dialect)
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[
    ...
    # Per-dialect shape is pinned in TestStatAggsPerDialect; here just confirm parseable SQL. Assert the call shape (qualified column in the arg slot), since family-name substrings pass even on regression.
    ...
    async def test_one_arg_stat_agg_generation(
        ...
        # The aggregate must wrap the resolved value column (orders.amount, since the `revenue` Column has sql="amount") in its single-arg slot.
    ...
    async def test_two_arg_stat_agg_generation(
        ...
        upper = sql.upper()
        assert "SELECT" in upper
        # Both legs (LHS column and other= kwarg) qualified, in the 2-arg slot in order; distinguishes this from the 1-arg test (Sonar S4144) and pins the 2-arg qualification path.
    ...
    async def test_dev1501_two_last_diff_time_cols_multi_dialect(
        ...
        """DEV-1501 cross-dialect: ``ORDER BY revenue:last(created_at) DESC, revenue:last(updated_at) ASC`` must materialise two distinct ranked aggregates per Tier-1 dialect, with dotted quoted aliases rendered correctly in the outer ORDER BY (no qualified-identifier misparse)."""
        m = SlayerModel(
            name="orders", sql_table="public.orders", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        # Two rankings, one per ordered aggregate, each in its own CTE — so neither the rn-suffix scheme nor any dialect-specific alias rewrite is in play (no dialect rewrites the ROW_NUMBER alias).
        ...
        # The outer ORDER BY must reference each materialised alias as a single dotted identifier, not split into relation.column. Filter to Column terms since MySQL emits synthetic CASE WHEN x IS NULL NULLS-LAST wrappers.
        tree = sqlglot.parse_one(sql, dialect=dialect)
        ...
        for ordered in order.expressions:
            ...
            if not isinstance(inner, sqlglot.exp.Column):
                continue
            # ``table`` is the qualified-prefix slot: either absent, or the ranked CTE's name. Anything else means the dotted alias was split and its first hop mistaken for a relation.
...
class TestStatAggsViaQueryEnrichment:
    """End-to-end aggregator-level checks (parser → enricher → generator). Confirms the new aggregations are reachable from query syntax, not just the internal _build_agg builder."""
    ...
    async def test_stat_agg_via_colon_syntax(
        ...
        # Pin the call shape: family name then the qualified column in its single-arg slot; sales.latency inside the call proves generation reached the value column.
    ...
    async def test_corr_via_colon_syntax_with_other_kwarg(
        ...
        sql = await _generate(generator=gen, query=query, model=sales_model)
        # Both legs flow through _resolve_sql and qualify under the LHS measure's model_name.
...
class TestPathAliasJoinInference:
    ...
    async def test_measure_sql_with_path_alias_infers_joins(
        ...
        """DEV-1502: measure SQL like ``customers__regions.population`` infers joins for both intermediate hops (symmetric to the dimension and time-dimension cases above)."""
        ...
        assert "customers" in join_aliases
        assert "customers__regions" in join_aliases
        # The aggregate body must reference the path-aliased ref — not the bare `region_pop_sum` (which would refer to a non-existent column on `orders`). Mirrors the DEV-1494 strengthening pattern.
        assert "SUM(customers__regions.population)" in sql


class TestMeasureSourceSqlJoinInference:
    """DEV-1502 → DEV-1709 (Stage 5): an AGGREGATE slot whose source ``Column.sql`` contains a ``__``-delimited join-path alias (or a sibling derived ref whose own sql does) ISOLATES into a host-rooted ``_cm_*`` CTE that pulls the implied LEFT JOINs inside its own scope (widened Law-3 trigger, D1) — so a measure-pulled join can never multiply the host rows seen by sibling measures. Row-level carriers (dimensions / WHERE filters) keep base-pulling their joins into the host ``_base`` (DEV-1484 / DEV-1494 — Law-1 territory)."""
    ...
    def _orders_model(self, *, extra_columns=None) -> SlayerModel:
        """Build the orders model with the standard customer/regions chain and any extra derived columns the per-test wants."""
        cols = [
            Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
            Column(name="customer_id", sql="customer_id", type=DataType.DOUBLE),
    ...
    # A first/last over a cross-join derived source isolates into a host-rooted _cm_* CTE; the crossed value is materialised as a _val_<n> projection instead of leaking the path-qualified ref. Local/bare sources need neither.
    @staticmethod
    def _assert_ref_only_in_val(sql: str, ref: str) -> None:
        """Assert ``ref`` (a cross-table-qualified column) appears ONLY as the source of a materialised ``<ref> AS _val_<n>`` projection inside the ranked subquery — i.e. it never leaks into an outer-scope aggregate body / HAVING / composite where it would fail at runtime with ``no such column``."""
        norm = _norm(sql)
    ...
    async def test_local_last_with_path_aliased_derived_source(
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        # The crossing source isolates. It is the RANKED route (DEV-1748): a first/last isolates because it ranks, which subsumes the crossing trigger this test was written for — the verdict is the same one.
    ...
    async def test_two_last_sharing_value_dedupe(
        ...
        """Two measures over the SAME crossing sql but DISTINCT derived columns intern to distinct ``AggregateKey``s (identity carries the column name), so each isolates into its OWN host-rooted CTE with its own ``_val`` materialisation — per-slot isolation, no CTE merging (DEV-1709 interview decision; merging is DEV-1688 / ``may_inline`` territory). Dedupe is therefore per-CTE."""
        model = self._orders_model(extra_columns=[
        ...
        # One _val materialisation per CTE, each consumed exactly once within its own scope (sibling CTEs are self-contained statements with scope-local ``_val`` numbering).
    ...
    async def test_same_sql_different_type_no_bad_dedupe(
        ...
        # The raw crossing ref must never surface in the combined outer SELECT (after the last CTE closes) — only the CTE output columns.
        outer = sql[sql.rfind("\n)") + 2:]
    ...
    async def test_composite_last_cross_join_source(
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        # Composite lowering (F3): the crossing first/last LEAF isolates; the `+ 1` composite renders in the combined SELECT via the CTE's projected alias.
        assert "_cm_" in sql, f"expected an isolation CTE:\n{sql}"
        self._assert_ref_only_in_val(sql, "customers__regions.payment_amount")
        outer = sql[sql.rfind("\n)") + 2:]
        assert "+ 1" in outer, (
    ...
    async def test_having_last_cross_join_source(
        ...
        """An aggregate-phase query filter referencing the isolated first/last routes to the OUTER WHERE on the combined SELECT (DEV-1503 rule; HAVING-into-the-CTE would surface host rows as NULL instead of dropping them)."""
        model = self._orders_model(extra_columns=[
            Column(name="region_payment", sql="customers.regions.payment_amount",
                   type=DataType.DOUBLE),
        ])
    ...
    async def test_val_alias_avoids_source_column_collision(
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        norm = _norm(sql)
        # The model's real `_val_0` column must not be shadowed: whatever alias the allocator picks for the materialisation, it is never `_val_0`, and the outer aggregate references the allocated one.
    ...
    async def test_local_only_derived_first_last_pulls_no_join(
        ...
        """The negative control: a LOCAL-only derived source (``amount * 2``) crosses no join, so the ranked CTE pulls none."""
        model = self._orders_model(extra_columns=[
    ...
    async def test_single_hop_path_alias_in_measure_sql(
        ...
        """A single-hop ``customers.region_id`` alias in the measure source SQL isolates host-rooted; the ``customers`` join lives inside the ``_cm_*`` CTE only."""
        model = self._orders_model(extra_columns=[
            Column(
    ...
    async def test_composite_arithmetic_path_aliased_measure(
        ...
        """``region_pop_sum:sum + amount:sum`` is rendered as a composite AGGREGATE slot (``ArithmeticKey`` wrapping two ``AggregateKey``s). The _visit recursion must descend through the operands so the path-aliased operand's joins surface."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_pop_sum",
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        # F3 composite lowering: the crossing leaf isolates into its own host-rooted CTE; the local leaf stays in the base; the `+` composite renders only in the combined SELECT.
        assert "_cm_" in sql, f"expected host-rooted isolation CTE:\n{sql}"
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
    ...
    async def test_composite_scalar_call_path_aliased_measure(
        ...
        """``coalesce(region_pop_sum:sum, 0)`` is a ``ScalarCallKey`` wrapping an ``AggregateKey``. The _visit recursion descends through ``key.args``."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_pop_sum",
                sql="customers.regions.population",
    ...
    async def test_mode_a_function_wrapping_path_alias_in_column_sql(
        ...
        """A Mode-A SQL function call wrapping a ``__`` ref in the column sql (``json_extract(customers__regions.props, '$.x')``) still triggers join discovery — ``_joined_paths_in_sql`` walks the parsed AST regardless of wrapper functions."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_prop_x",
        ...
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
        assert "LEFT JOIN customers" in cm_body, sql
        # The aggregate body wraps the JSON-extract call, qualified to the path alias (the bare alias `region_prop_x` would fail).
        assert "customers__regions.props" in cm_body, sql

    async def test_sibling_derived_chain_to_path_alias(
        self, engine: SlayerQueryEngine
    ) -> None:
        """``doubled_pop`` references sibling ``pop_helper``, whose own sql crosses the customers→regions join. ``_expand_derived_column_sql`` recursively inlines siblings; the new collector scans the recursively-expanded SQL."""
        model = self._orders_model(extra_columns=[
            Column(
                name="pop_helper",
                sql="customers.regions.population",
    ...
    async def test_post_transform_wrapping_path_aliased_measure(
        ...
        """A POST-phase transform like ``cumsum(region_pop_sum:sum)`` aux- materialises its inner ``AggregateKey`` into ``base_render_order``. The new collector visits aux slots the same way it visits public AGGREGATE slots, so the join discovery still fires."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_pop_sum",
                sql="customers.regions.population",
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        # The inner crossing aggregate isolates host-rooted; cumsum wraps the combined output. The path-aliased ref lives ONLY inside the `_cm_*` CTE body.
        assert "_cm_" in sql, f"expected host-rooted isolation CTE:\n{sql}"
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
        assert "customers__regions.population" in cm_body, sql
        assert "LEFT JOIN customers" in cm_body, sql
    ...
    async def test_multiple_aggregate_sources_sharing_join_dedupe(
        ...
        """Two DISTINCT crossing aggregates isolate into two separate host-rooted CTEs (per-AggregateKey-slot isolation), each pulling its own scope-local join set — so each hop's LEFT JOIN appears exactly once PER CTE and never in the host scope. Cross-CTE join sharing is optimizer territory (DEV-1688 / ``may_inline``)."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_pop_sum",
                sql="customers.regions.population",
    ...
    async def test_dim_and_measure_sharing_join_emits_join_once(
        ...
        """A joined DIMENSION base-pulls its join into the host scope (Law 1); the crossing MEASURE isolates with its own scope-local join set (Law 3). Each scope emits its joins exactly once: the ``customers`` hop appears once in the host and once inside the CTE; the deeper ``customers__regions`` hop only inside the CTE."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_pop_sum",
                sql="customers.regions.population",
        ...
        # Host scope: exactly one customers join (the dimension's), and never the deeper hop.
    ...
    async def test_filter_and_source_cross_different_joins(
        ...
        """Filter crosses the shallow join (customers) and source crosses the deeper join (customers__regions): both are crossing INPUTS of the same aggregate, so ONE host-rooted CTE owns both joins; the shared customers hop is emitted exactly once inside it, and the host scope stays join-free."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_pop_sum",
                sql="customers.regions.population",
                type=DataType.DOUBLE,
                # Filter crosses only the customers hop — does NOT touch customers__regions. Without DEV-1502 the regions join is missing entirely.
        ...
        assert "_cm_" in sql, f"expected host-rooted isolation CTE:\n{sql}"
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
        # customers join is emitted exactly once (inside the CTE) even though both the filter and the source discovered it.
        assert sql.count("LEFT JOIN customers AS customers") == 1, sql
        assert cm_body.count("LEFT JOIN customers AS customers") == 1, sql
        assert "customers__regions" in cm_body, sql
        # The filter CASE-WHEN references the customers hop. sqlglot may canonicalise ``IS NOT NULL`` to ``NOT ... IS NULL``, so check structurally rather than for an exact text form.
    ...
    async def test_no_path_alias_no_extra_join(
        ...
        """Sanity: an aggregate over a local column with no path alias in its sql emits zero joins (no spurious LEFT JOINs)."""
        model = self._orders_model()
        await engine.storage.save_model(model)
        query = SlayerQuery(
            source_model="orders",
    ...
    async def test_cross_model_source_not_in_host_collector_scope(
        ...
        """Defensive: the new collector skips AGGREGATE slots whose ``source.path != ()`` (cross-model). For a cross-model aggregate whose target column has a LOCAL sql (no further join) the host base FROM must not pull a spurious deeper join."""
        await storage.save_model(SlayerModel(
            name="orders", sql_table="orders", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        # Positive: cross-model CTE was actually produced; the count aggregate references the joined model's column inside it.
        assert "_cm_" in sql, f"expected cross-model CTE; got:\n{sql}"
        assert "customers.region_id" in sql
        # Negative: no spurious regions join leaks. A leak would show as path alias customers__regions; _join_aliases gives the exact set, so assert both bare and path shapes.
        join_aliases = _join_aliases(sql)
    ...
    async def test_cross_model_target_column_sql_crosses_further_join_xfail(
        ...
        """A cross-model aggregate ``customers_v2.deep_pop:sum`` where ``deep_pop`` lives on ``customers_v2`` with sql ``regions.population`` — the deeper customers_v2→regions join should appear inside the ``_cm_*`` CTE body. Currently it doesn't."""
        ...
        sql = (await engine.execute(query, dry_run=True)).sql
        # Extract the cross-model CTE body so the assertion ONLY pins the gap inside that scope (avoids accidental promotion from an unrelated host-level LEFT JOIN or formatting coincidence).
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
    ...
    async def test_agg_param_derived_column_path_alias_xfail(
        ...
        """``weighted_avg(weight=region_weight)`` where ``region_weight`` is a derived column with a path alias in its sql."""
        model = self._orders_model(extra_columns=[
            Column(
                name="region_weight",
                sql="customers.regions.weight",
        ...
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
        assert "LEFT JOIN customers" in cm_body, sql
        # The kwarg resolves to the EXPANDED sql inside the CTE, not the bare ident.
...
class TestDev1709WidenedIsolationShapes:
    """DEV-1709 (Stage 5) — generator shapes for the widened Law-3 trigger: co-occurrence with a local first/last (DEV-1702-B1), crossing explicit time args (D2 / DEV-1501 shape update), F4 host-filter inheritance into host-rooted CTEs, and the deferred implicit-time pin."""
    ...
    async def test_crossing_sum_coexists_with_local_last(self) -> None:
        """DEV-1702-B1: a regular crossing aggregate next to a local first/last isolates — it never renders in the ranked outer scope."""
        ...
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
        assert "SUM(customers__regions.population)" in cm_body, sql
        # Each aggregate owns a scope: the crossing SUM its own ``_cm_``, the first/last its own ``_cm_`` (DEV-1835 unified naming). Neither ref appears in the other's, and ``_base`` — which now holds nothing at all — pulls no join.
    ...
    async def test_crossing_kwarg_coexists_with_local_last(self) -> None:
        """A crossing KWARG next to a local first/last isolates the parametric aggregate the same way (Codex plan-review F4 fold-in: kwarg expressions must never leak into the ranked outer scope)."""
        orders = self._orders_model(extra_columns=[
    ...
    async def test_local_source_crossing_time_arg_isolates(self) -> None:
        """D2: ``amount:last(customers.signup_at)`` — a local source with a crossing explicit time arg isolates; the arg's join and ORDER BY ref live inside the CTE's ranked subquery (DEV-1501 shape update)."""
    ...
    async def test_user_template_fragment_kwarg_renders_join_in_cte(self) -> None:
        """A crossing USER template-fragment kwarg (``scale='customers__regions.weight'``) isolates AND the CTE sub-render registers the fragment's joins — the fragment renders qualified, so the LEFT JOINs must live inside the CTE body (PR #271 Codex review: fragments triggered isolation but their joins were never registered in the sub-render)."""
    ...
    async def test_model_default_fragment_kwarg_renders_join_in_cte(self) -> None:
        """Same for a crossing MODEL-DEFAULT ``AggregationParam.sql`` fragment — no user kwarg at all."""
        orders = self._orders_model(aggregations=[
            Aggregation(
    ...
    async def test_pathed_host_row_filter_inherited_into_cte(self) -> None:
        """F4 (Codex plan-review F5): a host ROW filter whose expression crosses a join constrains the host-rooted scope — it renders in BOTH the host base WHERE and the isolation CTE's scope, each with its own join registration."""
        orders = self._orders_model(extra_columns=[
            Column(name="region_pop_sum", sql="customers.regions.population",
                   type=DataType.DOUBLE),
        ])
    ...
    async def test_order_only_crossing_aggregate_isolates(self) -> None:
        """F3, hidden ORDER-only consumer: a crossing aggregate referenced ONLY by ORDER BY (colon-form ``OrderItem``) still isolates as a hidden slot; the ORDER BY reads the joined-back CTE column and the crossing ref never leaves the CTE."""
        orders = self._orders_model(extra_columns=[
            Column(name="region_pop_sum", sql="customers.regions.population",
                   type=DataType.DOUBLE),
        ])
    ...
    async def test_filter_only_composite_with_crossing_leaf(self) -> None:
        """F3, filter-only consumer: a composite referenced only by an aggregate-phase filter lowers the same way — crossing leaf isolates, the comparison lands in the combined outer WHERE (never HAVING), local operand promoted into ``_base``."""
        orders = self._orders_model(extra_columns=[
            Column(name="region_pop_sum", sql="customers.regions.population",
                   type=DataType.DOUBLE),
        ])
    ...
    async def test_ranked_isolation_survives_the_recursion_guard(self) -> None:
        """Two same-sql, different-CAST-type crossing first/lasts get a ranked producer scope EACH, and each materialises its own type-distinct value (DEV-1838 D4: the kernel form of the DEV-1709 type-distinctness pin)."""
        orders = self._orders_model(extra_columns=[
    ...
    async def test_a_ranked_sibling_no_longer_drags_a_kwarg_into_its_scope(
        ...
        """A crossing KWARG next to a first/last: the ranking has its own producer scope (DEV-1838 D4), so the crossing kwarg lives in ITS OWN producer and the ranked producer neither sees it nor is affected."""
        orders = self._orders_model(extra_columns=[
            Column(name="region_weight", sql="customers.regions.weight",
                   type=DataType.DOUBLE),
        ])
    ...
    # DEV-1835: local first/last desugar to the unified _cm_ producer and isolate universally; the crossing default-time first/last (formerly strict-xfail) now isolates.
...
class TestAggParamSanitization:
    ...
    async def test_filtered_custom_agg_does_not_case_wrap_literal_param(
        ...
        """A1 (CodeRabbit major on PR #82): when `_build_formula_agg` substitutes params into a filtered measure's template, literal defaults (e.g., `scale=100`) must stay literal. Wrapping them in `(CASE WHEN ... THEN 100 END)` turns a constant into a row expression — invalid GROUP-BY shape and semantically wrong. Row-level params (column refs) keep getting wrapped."""
        agg_model.aggregations = [
            Aggregation(
                name="scaled_sum",
        ...
        # Literal 100 must not be inside CASE WHEN; the value column must be. status is undeclared, so the Mode-A door qualifies it to the scope root (DEV-1745 W1).
...
class TestFilteredMeasures:
    ...
    async def _filtered_last_cross_model_sql(generator: SQLGenerator) -> str:
        """Shared setup for the two ``active_balance:last`` cross-model filter tests below. Builds the customers+orders models, runs enrichment with a stub join resolver, and returns the generated SQL — each caller asserts on a different facet of the output."""
        customers = SlayerModel(
            name="customers",
    ...
    async def test_filtered_last_with_cross_model_filter_carries_join(
        ...
        """Regression for CodeRabbit #8 — when a filtered last measure's filter references a column on a JOINED model, the LEFT JOIN must be applied INSIDE the ranked subquery so the filter columns resolve. Previously _build_last_ranked_from() built the subquery from base_from only and the outer string-level join injection never matched the subquery wrapper."""

        sql = await self._filtered_last_cross_model_sql(generator)
        # The customers LEFT JOIN must be inside the ranked subquery so the filter customers.status = 'active' resolves. Extract the subquery by matching balanced parens after `FROM (`.
        ...
        # The cross-model filter join appears in the ranked subquery (for filter resolution) and potentially in the isolated-measure CTE.
        assert "LEFT JOIN public.customers" in sql

    async def test_filtered_last_cross_model_isolates_to_cte_with_ranked_subquery(
        self, generator: SQLGenerator,
    ) -> None:
        """Regression for CodeRabbit B6-4 — when a filtered first/last measure's filter references a JOINED table (e.g. customers.status), the measure is isolated into a CTE of its own. The join lives inside the ranked subquery so the filter resolves, and the final SELECT does not reference the joined table directly."""

        sql = await self._filtered_last_cross_model_sql(generator)

        # The outermost SELECT (after all CTEs) should not reference 'customers.' directly — it pulls pre-computed values from CTEs.
    ...
    async def test_filter_with_dotted_string_literal_does_not_pull_spurious_join(
        ...
        """Regression for CodeRabbit #6 — when a measure filter contains a string literal that happens to include a dot (e.g. "url LIKE 'foo.bar%'"), the join planner must NOT mistake the literal for a `foo.<col>` ref and pull in an unwanted LEFT JOIN. The structured Mode-A predicate parse only lists real column references."""

        # Inline ModelJoin to a 'foo' model that is never registered. A regex over the filter string would match `foo.bar` inside the string literal and add 'foo' to needed_tables; structured parsing must not.
        orders = SlayerModel(
            name="orders",
            sql_table="public.orders",
            data_source="test",
...
class TestMeasureFilterInjection:
    ...
    async def test_drop_table_rejected(self, orders_model: SlayerModel) -> None:
        """Classic ``'; DROP TABLE ...`` payload is rejected by sqlglot when the WHERE clause is parsed under the target dialect."""
        orders_model.columns.append(
            Column(
                name="evil",
                sql="amount",
    ...
    async def test_embedded_single_quote_round_trips(
        ...
        """A SQL-escaped apostrophe (`O''Brien`) round-trips through the generator unchanged."""
        orders_model.columns.append(
            Column(
                name="irish_names",
                sql="amount",
    ...
    def _assert_round_trips_cleanly(sql: str, dialect: str) -> None:
        """Every emitted SQL string must tokenize + parse + round-trip in the target dialect. If a hostile filter manages to open an unclosed string literal, sqlglot's tokenizer raises ``TokenError`` — which is both the canonical pre-fix failure mode and a downstream DoS / error-leakage vector."""
        parsed = sqlglot.parse_one(sql, dialect=dialect)
        # Re-emitting must not raise either — guards against one-way tokenizer tolerance that wouldn't survive a round-trip through the planner.
    ...
    async def test_trailing_backslash_cannot_escape_closing_quote(
        ...
        """A trailing backslash in a string literal must not break out of the literal on escape-aware dialects (mysql, clickhouse, etc.)."""
        orders_model.columns.append(
            Column(
                name="evil",
                sql="amount",
        ...
        # Defence-in-depth: the a\ payload must stay one well-terminated literal; a single re-parse is idempotent after the dialect's unescaping.
    ...
    async def test_adversarial_quote_break_cannot_inject(
        ...
        """A backslash-quote injection payload must either be rejected at sqlglot generation time or be confined to a properly-terminated string literal that round-trips cleanly."""
    ...
    async def test_existing_filter_still_works_after_escaping(
        ...
        """Sanity: ordinary filters (no backslashes, no apostrophes) keep producing the same SQL shape after the escape-hardening change."""
        orders_model.columns.append(
            Column(name="active_revenue", sql="amount", filter="status = 'active'", type=DataType.DOUBLE)
        )
        query = SlayerQuery(
...
# Join target for TestSelfReferencingPaths, shared by the fixture (which saves it into the fixture's own store) and ``_engine_generate`` (which builds a fresh store per call and needs it passed as an extra model).
...
Column(name="score", sql="score", type=DataType.DOUBLE)],
    ...
    async def test_self_ref_dimension_resolved_after_strip(self, engine_and_models) -> None:
        ...
        # Resolved as a JOINED dimension under the dotted result key, not as a bare local column that silently referenced the wrong table.
...
Column(name="amount", sql="amount", type=DataType.DOUBLE)],
        ...
        # The constant should appear unqualified in WHERE. DEV-1361 wraps a non-bare ``Column.sql`` (literal ``1``) in CAST when ``type`` is set.
        assert (
            "1 = '1'" in sql
            or "1 = 1" in sql
...
Column(name="score", sql="score", type=DataType.DOUBLE)],
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            # rolling_avg lives 4 hops away (a → b → c → d → e). The function-style rewrite must still recognise it.
            query = SlayerQuery(
                source_model="a",
...
class TestParameterizedAggCanonicalDistinct:
    ...
    async def test_order_by_two_last_with_different_time_cols(
        ...
        """DEV-1501: two ORDER BY entries `revenue:last(created_at)` and `revenue:last(updated_at)` must produce DISTINCT ranked aggregates and not collapse onto one. The hidden materialised aggregates must be absent from the public projection (the result keys stay ``orders.status`` + ``orders._count``)."""
...
Column(name="revenue", sql="amount", type=DataType.DOUBLE)],
        ...
        async with _persist_and_engine(m) as engine:
            ...
            # The two outer ORDER BY EXPRESSIONS must NOT be byte-identical (the bug renders them identically; the fix gives each its own distinct ``_last_rn{suffix}``). Directions are independently asserted to remain DESC then ASC.
            ...
            # Hidden materialised aggregates must be TRIMMED from the public projection — the result keys are dim + count only.
            assert set(res.columns) == {"orders.status", "orders._count"}, (
                f"Hidden first/last aliases leaked into result columns: "
                f"{res.columns!r}\nSQL:\n{sql}"
            )
            # Response-meta invariant: the hidden materialised aliases must not appear under attributes.dimensions or attributes.measures either.
            attr_keys = (
...
async def _persist_and_engine(
    *models: SlayerModel,
    ...
    """Yield a ``SlayerQueryEngine`` with the given models persisted to a throwaway ``YAMLStorage``. Replaces the per-test ``tempfile + YAMLStorage + save_datasource + save_model + SlayerQueryEngine`` boilerplate that DEV-1501 repeated 16+ times — same shape, but one line at the call site. ``ds_name`` / ``ds_type`` follow the repo-wide convention (``data_source="test"`` everywhere)."""
    with tempfile.TemporaryDirectory() as tmp:
        storage = YAMLStorage(base_dir=tmp)
...
def _orders_with_paid_amount_model() -> SlayerModel:
    """DEV-1501 fixture-style helper — an ``orders`` model with a FILTERED ``paid_amount`` column (only ``status='paid'`` rows participate). Used by the filtered-first/last test suite (3+ tests share this exact model)."""
    return SlayerModel(
        name="orders", sql_table="orders", data_source="test",
        columns=[
            Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
...
def _orders_two_ts_model(*, default_td: "str | None" = None) -> SlayerModel:
    """DEV-1501 fixture-style helper — an ``orders`` model with TWO timestamp columns so first/last with different explicit time args can be exercised. ``default_td`` optionally sets ``default_time_dimension`` to test the "no explicit arg, fall back to default" path."""
    return SlayerModel(
        name="orders", sql_table="orders", data_source="test",
...
class TestDev1501HiddenFirstLastRender:
    """DEV-1501 — first/last aggregates reached only through ORDER BY, HAVING or a composite must still be materialised, must not collapse onto one another, and must not surface in the public projection."""

    @staticmethod
    def _ranked_ctes(sql: str) -> "list[str]":
        # Local first/last use the unified _cm_ CTE; cross-model first/last still isolates via _rk_. Match either prefix (all callers here are ranked-only).
        return _re.findall(r"(_(?:rk|cm)_\w+)\s+AS\s*\(", sql)

    async def test_order_by_first_and_last_different_time_cols(
        self, generator: SQLGenerator
    ) -> None:
        """ASC ``first(created_at)`` + DESC ``last(updated_at)`` produce distinct rank columns and distinct outer ORDER BY terms."""
        m = _orders_two_ts_model()
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            # The first(created_at) rank window orders created_at ascending (the bare column); the last(updated_at) one orders descending.
            norm = _norm(sql)
            assert "ORDER BY orders.created_at)" in norm, sql
            assert "ORDER BY orders.updated_at DESC)" in norm, sql
    ...
    async def test_order_by_first_and_last_same_time_col(
        ...
        """``first(created_at)`` ASC + ``last(created_at)`` DESC rank over the same column in opposite directions, so they are two aggregates and get two CTEs. The suffix bucketing this test was named for was the old scheme for keeping them apart inside one shared scope."""
        m = _orders_two_ts_model()
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
    ...
    async def test_order_by_last_with_explicit_time_no_default_time_dim(
        ...
        """Model has no ``default_time_dimension`` and the query has no temporal dimension; one ``ORDER BY revenue:last(created_at) DESC`` must still build the ranked subquery ordered by ``created_at``. Regression for the order-by-only + no-default case."""
    ...
    async def test_order_by_last_inherits_default_time_dim(
        ...
        """``ORDER BY revenue:last DESC`` (no explicit time arg) inherits ``default_time_dimension`` for the rank ordering."""
    ...
    async def test_projected_and_order_only_first_last_mixed(
        ...
        """A PROJECTED ``last(created_at)`` (alias ``latest_cr``) and an ORDER-BY-only ``last(updated_at)`` coexist — both rank columns exist, the projected alias surfaces in result keys, the order-only materialised alias is trimmed from result keys."""
        m = _orders_two_ts_model()
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            # The two aggregates must not collapse: each ranks by its own time column in its own CTE. Read off the windows — no shared alias space remains to hide a collapse.
            norm = _norm(sql)
            assert "ORDER BY orders.created_at DESC" in norm, sql
            assert "ORDER BY orders.updated_at DESC" in norm, sql
            # The PROJECTED one surfaces under the user's alias; the order-only one is named only by the ORDER BY.
    ...
    async def test_filter_only_last_with_having(
        ...
        """A filter referencing a non-projected ``last(created_at)`` must materialise that aggregate and apply the predicate where its value is readable — the outer combined SELECT."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
    ...
    async def test_filter_only_last_two_time_cols(
        ...
        """A filter references TWO last() aggregates over different time columns — each is materialised in its own CTE and the outer predicate names both, so distinct time columns cannot collapse."""
        m = _orders_two_ts_model()
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
    ...
    async def test_filter_and_order_last_different_time_cols(
        ...
        """A filter uses ``last(created_at)`` while ORDER BY uses ``last(updated_at)`` — each must resolve to its OWN materialised value. Combined-surface regression (Codex MED 8)."""
        m = _orders_two_ts_model()
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            # Outer ORDER BY references the materialised alias for the updated_at first/last.
            terms = _outer_order_terms(sql)
    ...
    async def test_projected_two_last_different_time_cols_no_default(
        ...
        """Two PROJECTED ``last(created_at)`` / ``last(updated_at)`` on a model with NO ``default_time_dimension`` must stay DISTINCT."""
        m = _orders_two_ts_model()  # no default_td
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            assert "ORDER BY orders.created_at DESC" in norm, sql
            assert "ORDER BY orders.updated_at DESC" in norm, sql
            # …and each user alias is projected from a DIFFERENT CTE, which is the collapse this test exists to catch.
    ...
    async def test_filtered_first_last_in_nested_having_uses_filtered_rn(
        ...
        """A FILTERED ``last(time_col)`` nested inside a scalar-function call (``coalesce(agg, 0)``) must still resolve to the aggregate's own materialised value through the recursion."""
        async with _persist_and_engine(_orders_with_paid_amount_model()) as engine:
            # ``coalesce(agg, 0)`` wraps the AggregateKey in a ScalarCallKey — exercises the recursion branch that previously dropped ``aliases_by_slot_id``.
            query = SlayerQuery(
                source_model="orders",
                measures=["*:count"],
                dimensions=[ColumnRef(name="status")],
    ...
    async def test_composite_only_first_last_triggers_ranked_subquery(
        ...
        """A query whose ONLY first/last reference is INSIDE a composite aggregate (no direct first/last sibling) must still materialise each operand. Codex review of DEV-1501 PR #159 round 4."""
    ...
    async def test_composite_first_last_without_default_time_dim_raises(
        ...
        """A composite measure containing a first/last operand without explicit time arg AND no model ``default_time_dimension`` must raise the "first/last requires a ranking time column" error. Without composite-aware validation in ``_build_first_last_base_select``, the query bypasses the check and ``_build_unfiltered_rn_columns`` emits ``ORDER BY None``. Codex review of DEV-1501 PR #159 round 5."""
        # ``_orders_two_ts_model()`` has no ``default_time_dimension``.
        async with _persist_and_engine(_orders_two_ts_model()) as engine:
            query = SlayerQuery(
                source_model="orders",
                # Composite with bare ``revenue:last`` — no explicit time arg, no default time dim, no temporal dim.
                measures=[ModelMeasure(
    ...
    async def test_composite_first_last_with_joined_time_arg_adds_join(
        ...
        """A composite measure containing a first/last operand with a JOINED explicit time arg (``revenue:last(customers.signed_up_at) + 1``) must pull the ``customers`` join into the FROM so the ranked subquery's ORDER BY can reference ``customers.signed_up_at``. Without composite-aware path discovery in ``_collect_joined_paths_for_base``, the join is missing and the SQL references an unjoined alias. Codex review of DEV-1501 PR #159 round 5."""
        customers = SlayerModel(
            name="customers", sql_table="customers", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        async with _persist_and_engine(customers, orders) as engine:
            ...
            sql = (await engine.execute(query, dry_run=True)).sql
            _assert_valid_sql(sql, dialect=generator.dialect)
            # The ranked subquery's ORDER BY references customers.signed_up_at, so the customers join MUST be in the FROM.
    ...
    async def test_composite_first_last_in_projection_uses_correct_suffixes(
        ...
        """A COMPOSITE aggregate measure containing first/last operands with different explicit time columns (``revenue:last(created_at) + revenue:last(updated_at)``) must render each operand against its OWN ranking, alongside a directly projected first/last that shares one of those time columns."""
        async with _persist_and_engine(_orders_two_ts_model(default_td="created_at")) as engine:
            query = SlayerQuery(
                source_model="orders",
                # Direct first/last triggers _build_first_last_base_select; the composite operand is the bug under test.
                measures=[
                    ModelMeasure(formula="revenue:last(created_at)", name="lc"),
                    ModelMeasure(
                        formula="revenue:last(created_at) + revenue:last(updated_at)",
            ...
            sql = (await engine.execute(query, dry_run=True)).sql
            _assert_valid_sql(sql, dialect=generator.dialect)
            # ``lc`` and the composite's created_at operand are the SAME aggregate (one structural key, one slot, one CTE); the updated_at operand is a second one.
            names = self._ranked_ctes(sql)
            assert len(names) == 2, sql
            diff_match = _re.search(
                r'(_cm_\w+)\."[^"]+" \+ (_cm_\w+)\."[^"]+" AS "orders\.diff"',
    ...
    async def test_cross_model_query_with_local_first_last_having_filter(
        ...
        """A query carrying BOTH a cross-model aggregate AND a LOCAL first/last filter must emit valid SQL."""
        customers = SlayerModel(
            name="customers", sql_table="customers", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        async with _persist_and_engine(customers, orders) as engine:
            ...
            sql = (await engine.execute(query, dry_run=True)).sql
            _assert_valid_sql(sql, dialect=generator.dialect)
            # Three scopes, each owning one thing: ``_base`` the grain, the cross-model SUM's ``_cm_`` CTE, and the ranking's own ``_cm_`` CTE (DEV-1835 unified naming; the ranked one carries ``_last``).
            ...
            # The predicate on the ranked value is an outer WHERE, never a HAVING that a LEFT JOIN back would turn into a NULL row.
            assert "HAVING" not in sql.upper(), sql
            tail = sql[sql.rfind("WHERE"):]
            assert tail.startswith("WHERE"), sql
            assert "> 100" in tail, sql
    ...
    async def test_cross_model_filtered_last_in_having(
        ...
        customers = SlayerModel(
            ...
            columns=[
                ...
                # Filtered column — only active customers' rows rank. active is BOOLEAN, so use = TRUE (Postgres rejects active = 1) for a valid CASE WHEN.
                Column(
    ...
    async def test_derived_time_arg_pulls_in_referenced_join(
        ...
        """DEV-1501 (Codex round 8): a DERIVED first/last time arg whose ``Column.sql`` references a joined column must pull that join into the base FROM. Join discovery (``_resolve_agg_inputs_via_scope`` via ``_explicit_time_arg_of``) registers it, and the ranked plan expands ``net_signed_at.sql = "customers.signed_up_at"`` so the ranked subquery's ``ORDER BY`` emits ``customers.signed_up_at``; without a corresponding ``LEFT JOIN customers`` the SQL is broken. Previously ``_collect_joined_paths_for_base`` only walked ``ColumnKey`` args; ``ColumnSqlKey`` derived time args were invisible to join discovery."""
        customers = SlayerModel(
            name="customers", sql_table="customers", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        orders = SlayerModel(
            ...
            columns=[
                ...
                Column(name="status", sql="status", type=DataType.TEXT),
                Column(name="amount", sql="amount", type=DataType.DOUBLE),
                # Derived time column whose sql crosses the customers join. No other slot drags ``customers`` into the FROM, so this is the ONLY signal join discovery has to add it.
                Column(
        ...
        async with _persist_and_engine(customers, orders) as engine:
            ...
            sql = (await engine.execute(query, dry_run=True)).sql
            _assert_valid_sql(sql, dialect=generator.dialect)
            # The ranked subquery's ORDER BY must reference the expanded joined column.
            assert "customers.signed_up_at" in sql, (
                f"Expected expanded customers.signed_up_at ranking expression in SQL:\n{sql}"
            )
            # The customers join must be present in the base FROM. Without the DEV-1501 fix, ``_collect_joined_paths_for_base`` skipped the ColumnSqlKey arg and the join was missing → broken SQL.
            assert _re.search(
...
class TestDev1501BroadTriggerAndGuards:
    """DEV-1501 — broad-trigger materialise+trim behaviour and the guards Codex flagged: the wrap is conditional, hidden ROW order targets must not be materialised (cardinality preserved), composite hidden order is explicitly NotImplementedError, dim-only-dedup grain is preserved."""

    async def test_outer_wrap_only_when_hidden_present(
        self, generator: SQLGenerator
    ) -> None:
        """When no hidden materialised aggregates exist, the no-transform path stays flat — no outer trim wrapper. ORDER BY references the projected alias inline at the same SELECT level."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            sql = (await engine.execute(query, dry_run=True)).sql
            _assert_valid_sql(sql, dialect=generator.dialect)
            # No hidden materialised aggregates → top-level FROM is a bare Table (no outer wrap Subquery wrapper).
    ...
    async def test_order_by_hidden_simple_aggregate_materialized(
        ...
        """Broad-trigger regression: a NON-first/last hidden order aggregate (``ORDER BY revenue:sum DESC`` with no projected ``revenue:sum``) also goes through materialise+trim — outer wrap projects only the public columns, outer ORDER BY references the materialised ``"orders.revenue_sum"`` alias, and the hidden alias is absent from result keys and response metadata."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            # Outer ORDER BY references the materialised alias EXACTLY as the full dotted form ``"orders.revenue_sum"`` (quoted dotted alias body, not qualified ``"orders"."revenue_sum"``).
            terms = _outer_order_terms(sql)
            assert len(terms) == 1, terms
    ...
    async def test_dim_only_dedup_with_hidden_order_first_last(
        ...
        """Dim-only query (no measures, dim auto-dedup) with a hidden first/last in ORDER BY. The dim-only-dedup GROUP BY must contain ONLY the dimension(s) — the aggregate-only narrowing of Change 2 prevents the hidden first/last from leaking extra GROUP BY entries."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            # Every Select that has a GROUP BY must group by exactly ONE expression (the status dim). The inner ranked subquery has no GROUP BY; the base SELECT groups by status; the outer wrap has no GROUP BY.
            group_counts = [
    ...
    async def test_hidden_row_order_target_max_wraps_without_widening_grain(
        ...
        """ORDER BY a non-projected LOCAL ROW column (e.g. ``customer_id``) in an aggregated query."""
        m = SlayerModel(
            name="orders", sql_table="orders", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
    ...
    async def test_hidden_simple_aggregate_in_having(
        ...
        """``filters=["revenue:sum > 100"]`` with no projected ``revenue:sum`` (plain non-first/last). Broad materialise+trim applies to filter-side too: hidden ``revenue:sum`` materialised in base, HAVING references it (inline or by alias), absent from result keys and response metadata."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
    ...
    async def test_order_by_with_limit_offset_on_outer_wrap(
        ...
        """ORDER BY / LIMIT / OFFSET must be applied where the ordered value is visible — never below it, which would slice rows before the aggregate being sorted on exists."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
            ...
            # ...and the ORDER BY reads the ranked CTE, so the pagination is applied above the value it sorts on.
    ...
    async def test_c13_duplicate_aliases_with_outer_wrap(
        ...
        """DEV-1450 C13 — two declared measures with the same structural key but different ``name``s intern to ONE slot carrying TWO ``public_aliases``. Under the new outer trim wrap, BOTH aliases must appear in the outer projection (mirroring the transform path's ``outer_alias_index``)."""
        m = _orders_two_ts_model(default_td="created_at")
        async with _persist_and_engine(m) as engine:
            query = SlayerQuery(
                source_model="orders",
                # Same key (revenue:sum), two different names → one interned slot with two public_aliases.
                measures=[
            ...
            sql = res.sql
            _assert_valid_sql(sql, dialect=generator.dialect)
            # Both C13 aliases must be in the public projection — the outer wrap must not collapse them to one.
    ...
    async def test_composite_filter_materialises_aggregate_leaves(
        ...
        """A composite HAVING filter (``revenue:sum - cost:sum > 0``) with neither operand projected: each AggregateKey leaf must be materialised in the base SELECT (composites recurse into operands per ``_iter_slot_deps``), HAVING references them (inline or by alias), the composite expression itself is inlined. The hidden leaf aliases are trimmed from result keys; no row leaves leak into GROUP BY."""
        m = SlayerModel(
            name="orders", sql_table="orders", data_source="test",
            default_time_dimension="created_at",
            columns=[
        ...
        async with _persist_and_engine(m) as engine:
            ...
            # HAVING present, references both operand aggregates (inline or by materialised alias). Either SUM(orders.amount) or "orders.revenue_sum" / "orders.cost_sum" — the test asserts both source columns are reachable from HAVING.
...
class TestMultiHopCrossModelMeasure:
    ...
    async def test_two_hop_measure(self, generator: SQLGenerator) -> None:
        """policy_coverage_detail.claim_coverage.claim_amount.total_claim_amount:sum should walk policy_coverage_detail → claim_coverage → claim_amount."""
...
Column(name="total_policy_amount", sql="policy_amount", type=DataType.DOUBLE)],
    ...
    async def test_rerooted_cte_includes_reachable_and_sibling_filters(self, generator, _models):
        """DEV-1836 D3: the re-rooted producer includes filters REACHABLE from the target root — the target-path filter (``policy_amount.premium.has_premium``) AND the host-sibling filter (``agreement_party_role``): the root's own provably to-one join reaches the sibling, so it inherits instead of dropping (the Q9 shape). Nothing warns."""
        ...
        async with self._setup_engine(policy, policy_amount, premium, agreement_party_role) as engine:
            ...
            # Only the producer CTE body — the outer SELECT would satisfy these assertions even with filter inheritance missing from the CTE.
...
Column(name="score", sql="score", type=DataType.DOUBLE)],
    ...
    async def test_rerooted_local_filter_remapped_to_source(self, generator, _models):
        """Unqualified filter on source model is remapped to source.col in CTE (the producer reaches the host over its provably to-one reverse join)."""
        policy, policy_amount, premium, agreement_party_role = _models
        async with self._setup_engine(policy, policy_amount, premium, agreement_party_role) as engine:
            ...
            # Only the producer CTE body — the outer SELECT can also carry the filter, so the whole-tail slice would pass vacuously.
            cte_body = _extract_cte_body(sql=sql, cte_name_pattern=r"_cm_\w+")
...
class TestOrderByCustomFieldName:
    ...
    async def test_order_by_custom_name(self, generator: SQLGenerator) -> None:
        """Field with custom name 'num_customers' is the surfaced alias and ORDER BY references it directly (DEV-1335 — user ``name`` overrides the canonical ``customer_id_count_distinct`` form)."""
        model = SlayerModel(
            name="orders",
            sql_table="orders",
            data_source="test",
...
Column(name="revenue", sql="amount", type=DataType.DOUBLE)],
        ...
        assert "ORDER BY" in sql
        order_clause = sql.split("ORDER BY", 1)[1]
        # User name is the ORDER BY column (DEV-1335 — user ``name`` overrides the canonical form).
        assert '"orders.num_customers"' in order_clause, (
...
class TestMeasureFilterCrossModelJoin:
    ...
    async def test_measure_filter_cross_model_constant_triggers_join(self, generator: SQLGenerator) -> None:
        ...
        # The JOIN to loss_payment must be present (it is the filter mechanism). Parsed-alias assertion so a stray table mention / JOIN keyword in unrelated SQL can't satisfy it.
        assert "loss_payment" in _join_aliases(sql), f"Missing JOIN to loss_payment: {sql}"
        # The derived ref must be inlined — no dangling ``loss_payment.has_flag`` (a derived column, absent from the physical table).
        ...
        # The inlined constant must live INSIDE the aggregation-time CASE-WHEN wrapper (not relocated to WHERE), guarding the column-filter shape.
...
class TestIsolatedFilteredMeasureCTEs:
    ...
    async def _sql(self, claim_amount_model, related_models, query):
        """Run ``query`` against ``claim_amount_model`` (with the related join targets registered) through the typed engine and return emitted SQL."""
        return await _engine_generate(
            query=query,
    ...
    async def test_two_filtered_measures_get_separate_ctes(
        ...
        # The host _base CTE must NOT have both INNER filter-target JOINs (legacy bug: intersecting to zero rows). Inspect the _base body.
    ...
    async def test_formula_over_isolated_measures(
        ...
        """Formula referencing isolated measures evaluates against the joined-back ``_cm_*`` columns at the combined SELECT — NOT inline in ``_base`` (which would pull both filter-target INNER joins back into the host CTE and intersect to rows present in BOTH targets, silently corrupting both aggregates)."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[
        ...
        # Both filter-target INNER joins must live INSIDE the per-measure ``_cm_*`` CTEs, NEVER in the host ``_base`` body. Inline rendering of the composite would re-introduce them and the intersection bug DEV-1503 set out to eliminate.
        base_body = _extract_cte_body(sql, r"_base")
        ...
        # The composite alias must appear in the OUTER combined SELECT (after all CTEs), referencing both ``_cm_*`` joined-back columns.
    ...
    async def test_aggregate_filter_on_isolated_measure_applied_on_outer(
        ...
        """Aggregate-phase filter referencing an isolated measure is applied on the outer combined SELECT (as WHERE), not inside the _cm_ CTE."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[ModelMeasure(formula="loss_payment_amt:sum")],
            dimensions=[ColumnRef(name="claim.claim_number")],
        ...
        # The comparison applies on the OUTER combined SELECT (after all CTEs) — locate the position of the last CTE-close ``\n)`` and assert the filter literal appears after it.
        cm_match = _re.search(r"_cm_\w+\s+AS\s*\(", sql)
        assert cm_match, f"Expected _cm_ CTE in:\n{sql}"
        # Find the last ``\n)`` that closes a top-level CTE — the segment AFTER it is the outer combined SELECT.
        last_cte_close = sql.rfind("\n)")
        ...
        # And that outer comparison must be a WHERE, not a HAVING (the outer SELECT is not aggregating; HAVING without GROUP BY is invalid SQL on every supported dialect).
    ...
    async def test_isolated_last_no_ranked_subquery_in_base(
        ...
        """When ALL first/last measures are isolated, the host _base CTE must NOT build a ranked subquery — it should be a plain dimension spine. The ranked subquery lives inside each filtered measure's own _cm_ CTE."""
        query = SlayerQuery(
            source_model="claim_amount",
        ...
        # Extract the _base CTE body (balanced-paren walker — guards against nested ranked subqueries in mixed-isolation cases).
        base_body = _extract_cte_body(sql, r"_base")
    ...
    async def test_isolated_last_cte_has_valid_ranked_subquery(
        ...
        """The isolated CTE for a last measure must contain a ROW_NUMBER ranked subquery and produce valid SQL — no reference to a rank column its FROM never projects."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[ModelMeasure(formula="latest_payment:last")],
            dimensions=[ColumnRef(name="claim.claim_number")],
        ...
        # The _cm_ CTE for the isolated measure must exist and contain ROW_NUMBER. Balanced-paren walker — the body carries a nested ranked subquery whose ``\n)`` closes the inner subquery, not the CTE.
    ...
    async def test_isolated_first_with_explicit_time_column(
        ...
        # The ranked CTE orders ASCENDING for ``first`` — the bare column, since ascending is the absence of DESC.
    ...
    async def test_multiple_isolated_first_last_separate_ctes(
        ...
        # Each should have ROW_NUMBER. Balanced-paren walker because each ``_cm_*`` body wraps a ranked subquery.
    ...
    async def test_multi_hop_derived_filter_expands_inside_cm_cte(
        ...
        """A ``Column.filter`` referencing a host DERIVED column whose own ``Column.sql`` crosses TWO join hops (``customers.regions.code``) must expand inside the isolated _cm_ CTE — both join hops must appear in the CTE body, never in the host _base spine."""
        regions = SlayerModel(
            name="regions", sql_table="Regions", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
        ...
        orders = SlayerModel(
            ...
            columns=[
                ...
                Column(name="customer_id", sql="customer_id", type=DataType.DOUBLE),
                Column(name="amount", sql="amount", type=DataType.DOUBLE),
                # Derived column reaching `customers__regions.code` via a 2-hop walk (Mode A uses ``__`` between hops + single dot before the leaf).
        ...
        # The _cm_ CTE must exist for the filtered measure — balanced-paren walker so a nested subquery doesn't truncate the body.
    ...
    async def test_aggregate_filter_outer_where_with_no_dimensions(
        ...
        """Aggregate filter on an isolated measure, with NO dimensions: the host _base CROSS JOINs the _cm_ CTE, and the outer wrapper applies the filter as WHERE on the joined-back aggregate column."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[ModelMeasure(formula="loss_payment_amt:sum")],
            filters=["loss_payment_amt:sum > 1000"],
    ...
    async def test_mixed_operand_aggregate_filter_promotes_hidden_operand(
        ...
        """A filter that references BOTH an isolated filtered-local aggregate AND a non-isolated local aggregate must promote the non-isolated operand to a hidden column in the host _base so the outer WHERE can evaluate it. The non-isolated operand is NOT a public measure here — it must stay hidden from the final projection."""
        # total_amount is a non-isolated unfiltered local measure — referenced only by the filter, NOT projected as a public measure.
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[ModelMeasure(formula="loss_payment_amt:sum")],
            dimensions=[ColumnRef(name="claim.claim_number")],
        ...
        assert "1000" in sql
        assert "10" in sql
        # DEV-1838 D5: each conjunct routes to its own scope — the local operand stays a HAVING on _base (value-identical to the old outer-WHERE), the isolated aggregate's goes to the outer combined WHERE.
        base_body = _extract_cte_body(sql, r"_base")
        ...
        # The outer combined SELECT (after all CTEs) applies the isolated aggregate's conjunct against the joined-back ``_cm_`` column.
        last_cte_close = sql.rfind("\n)")
    ...
    async def test_post_phase_filter_not_routed_to_outer_combined_where(
        ...
        """A POST-phase filter (transform-wrapped, ``cumsum(...) > 0``) on an isolated filtered-local measure stays in the existing post-transform wrapper — it must NOT be re-routed to the outer combined WHERE."""
    ...
    async def test_filter_referencing_two_isolated_aggregates(
        ...
        """An aggregate-phase filter referencing TWO isolated filtered-local aggregates (``loss_payment_amt:sum + loss_reserve_amt:sum > 100``) must produce two _cm_ CTEs (one per measure) and an outer WHERE that references BOTH joined-back aliases."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[
                ModelMeasure(formula="loss_payment_amt:sum"),
    ...
    async def test_filtered_local_parametric_last_in_order_by(
        ...
        """A filtered-local PARAMETRIC ``last(updated_at)`` referenced from ORDER BY keeps its ranking INSIDE its own CTE, and the outer ORDER BY resolves through that CTE's joined-back column."""
        claim_amount_model_with_time.columns.append(
            Column(name="updated_at", sql="updated_at", type=DataType.TIMESTAMP),
        )
        query = SlayerQuery(
        ...
        # The window's ORDER BY must use updated_at (the explicit time arg), not the default TD (created_at).
        assert "ORDER BY claim_amount.updated_at DESC" in _norm(cm_body), (
            f"the ranked CTE must rank by the explicit time arg:\n{cm_body}"
        )
        # The outer ORDER BY references the joined-back aggregate column — NOT the rank column, which is scoped to the CTE.
    ...
    async def test_filtered_local_parametric_last_in_having(
        ...
        """A filtered-local PARAMETRIC ``last(updated_at)`` referenced from a host filter routes as an AGGREGATE-phase filter to the outer combined WHERE. The ranking stays inside the CTE; the outer SELECT applies the comparison against the joined-back column."""
        claim_amount_model_with_time.columns.append(
            Column(name="updated_at", sql="updated_at", type=DataType.TIMESTAMP),
        )
        query = SlayerQuery(
        ...
        # The aggregate-phase filter routes to the OUTER combined SELECT wrapper (WHERE on the joined-back column), not inside the CTE and not as HAVING.
        last_cte_close = sql.rfind("\n)")
        outer = sql[last_cte_close + 2:]
    ...
    async def test_outer_wrapper_resolves_forward_cross_model_agg_operand(
        ...
        """A mixed AGGREGATE-phase filter referencing BOTH an isolated filtered-local aggregate AND a forward cross-model aggregate must resolve BOTH operands at the outer combined SELECT — the filtered-local through the host-rooted ``_cm_`` CTE, the forward cross-model through ITS own ``_cm_`` CTE."""
        # ``loss_payment.has_flag:sum`` is a forward cross-model aggregate on the joined ``loss_payment`` target — no Column.filter, so it routes as a plain forward-path ``_cm_`` plan, not filtered-local.
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[
                ModelMeasure(formula="loss_payment_amt:sum"),
        ...
        sql = await self._sql(claim_amount_model, related_models, query)
        # Two _cm_ CTEs: one filtered-local (host-rooted), one forward cross-model.
        cm_cte_names = _re.findall(r"(_cm_\w+)\s+AS\s*\(", sql)
        assert len(cm_cte_names) >= 2, (
            f"Expected ≥ 2 _cm_ CTEs (filtered-local + forward); got {cm_cte_names}\n{sql}"
        )
        # Outer combined SELECT must carry the filter and reference both aggregate aliases.
        last_cte_close = sql.rfind("\n)")
        outer = sql[last_cte_close + 2:]
        assert "100" in outer, (
    ...
    async def test_aggregate_filter_with_date_range_routes_correctly(
        ...
        """A query carrying BOTH a date_range time dimension AND an AGGREGATE-phase filter on the isolated aggregate must route each independently: the date_range as ROW-phase WHERE (propagated into the sub-plan / _cm_ CTE), the aggregate filter as outer WHERE on the joined-back column."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[ModelMeasure(formula="loss_payment_amt:sum")],
            dimensions=[ColumnRef(name="claim.claim_number")],
        ...
        sql = await self._sql(claim_amount_model_with_time, related_models, query)
        # The date_range literal is a ROW-phase filter — must apply where the row-set forms (inside the _cm_ CTE for the isolated agg).
        cm_body = _extract_cte_body(sql, r"_cm_\w+")
        assert "2020-01-01" in cm_body, (
            f"date_range ROW filter must apply inside the _cm_ CTE:\n{cm_body}"
        )
        # The AGGREGATE filter must apply on the OUTER combined SELECT, not inside the CTE (which would surface host rows as NULL via LEFT JOIN instead of dropping them).
        last_cte_close = sql.rfind("\n)")
        outer = sql[last_cte_close + 2:]
        assert "1000" in outer, (
        ...
        # Double-application check: the '> 1000' literal must NOT also appear inside the _cm_ CTE body (which would mean the planner mis-routed the aggregate filter as ROW phase under the date_range off-by-one bug).
    ...
    async def test_order_by_projected_composite_over_isolated_resolves_at_combined(
        ...
        """A query whose ORDER BY references a PROJECTED composite over isolated filtered-local aggregates must resolve the order alias at the combined SELECT level — NOT ``_base."<alias>"`` (the alias is no longer materialised in ``_base`` after DEV-1503 routing)."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[
                ModelMeasure(formula="loss_payment_amt:sum"),
    ...
    async def test_outer_composite_with_multiple_user_aliases(
        ...
        """The same composite formula declared under TWO user-aliases must surface BOTH aliases in the combined SELECT. The C13 multi-alias same-key consolidation collapses the two measures into ONE slot with ``public_aliases=[a, b]``; the renderer must cycle through them rather than emit ``public_aliases[0]`` twice."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[
                ModelMeasure(formula="loss_payment_amt:sum"),
    ...
    async def test_local_first_last_with_routed_cross_model_filter(
        ...
        """A query mixing a LOCAL first/last measure with a cross-model aggregate carrying a routed filter must NOT apply the routed filter inside the host ``_base`` ranked subquery — it belongs in the ``_cm_*`` CTE only."""
        # total_amount (local, first()) forces _base into the ranked-subquery branch; loss_payment.has_flag is forward cross-model, and its > 0 filter classifies as PROPAGATE_HAVING there.
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[
                ModelMeasure(formula="total_amount:last"),
        ...
        # DEV-1836: the routed cross-model aggregate-phase filter lands as an OUTER WHERE on the producer's attached value — once, and never inside the host ``_base`` ranked subquery (double-application bug).
        norm = _norm(sql)
    ...
    async def test_no_dim_query_with_host_row_filter_applies_in_base(
        ...
        """A no-dimension query with cross-model aggregates AND a host-local ROW filter must apply the filter at the placeholder ``_base`` (via ``WHERE`` + ``LIMIT 1``). The round-2 fix that dropped the host ``FROM`` to avoid N-row CROSS JOIN duplication also bypassed ``_build_where_having_from_planned``, silently ignoring host-local ROW filters (Codex round 4 / CodeRabbit)."""
        ...
        # The ``_base`` placeholder must carry the filter as WHERE plus LIMIT 1 (the 1-row cardinality preservation from round 2).
        base_body = _extract_cte_body(sql, r"_base")
    ...
    async def test_no_dim_host_filter_referencing_joined_column_pulls_join(
        ...
        """A no-dimension query with a host ROW filter that references a JOINED column (``claim.claim_number = '...'``) must pull the join into the placeholder ``_base`` FROM clause — the WHERE otherwise references an undefined alias."""
        query = SlayerQuery(
            source_model="claim_amount",
            measures=[ModelMeasure(formula="loss_payment_amt:sum")],
            filters=["claim.claim_number = '12345'"],
    ...
    async def test_first_last_with_host_filter_not_reapplied_outside_ranked_subquery(
        ...
        """A query mixing a LOCAL first/last measure and a host ROW filter must apply the filter only INSIDE the ranked subquery (pre-ranking), not again on the outer ``_base`` SELECT. The first/last branch returns ``where_consumed=True``; the cross- model orchestrator must honour that and skip the outer WHERE application, otherwise the filter double-applies and changes first/last semantics by filtering AFTER ranking."""
        claim_amount_model_with_time.columns.append(
            Column(name="status", sql="status", type=DataType.TEXT),
        )
        query = SlayerQuery(
        ...
        # The filter literal must appear exactly once in _base — inside the ranked subquery's WHERE; a second occurrence means base_where was wrongly re-applied.
        base_body = _extract_cte_body(sql, r"_base")
    ...
    async def test_filtered_local_in_source_queries_smoke(
        ...
        """A query-backed model containing a filtered-local measure must compile cleanly — the host-rooted _cm_ CTE renders against the virtual (sql-mode) host without tripping ``_build_from_clause_from_planned``."""
        loss_payment = SlayerModel(
            name="loss_payment", sql_table="Loss_Payment", data_source="test",
            columns=[
                Column(name="claim_amount_id", sql="Claim_Amount_Identifier", type=DataType.DOUBLE, primary_key=True),
        ...
        # Auxiliary base model the subquery host references; the filtered-local CTE root path must not trip on a non-sql_table host shape.
        ...
        # Use a model with `sql` (subquery) directly — simpler than source_queries plumbing, but still exercises the non-sql_table host-rooted CTE path.
...
class TestGetColumnTypesSql:
    ...
    async def test_expression_measure_sql_not_corrupted(self) -> None:
        ...
        with patch.object(storage, "get_model", new_callable=AsyncMock, return_value=model):
            ...
            mock_ds.type = "sqlite"
            # Real attribute, not an auto-Mock: it feeds the cache key's credential digest, which hashes it.
        ...
        assert "orders.amount" in sql
        # Bare ``amount`` inside the expression now qualifies to the model's alias (``orders.amount``) — derived-ref expansion (DEV-1333) makes every base-column reference unambiguous.
        assert "COALESCE(orders.amount, 0)" in sql

    async def test_cross_model_measures_probed_via_engine(self) -> None:
        """Cross-model-derived columns are included in the type probe and mapped back to their bare column names. ``customer_score`` is a local column whose SQL reaches the joined ``customers`` model; the typed probe pipeline must still build a ``customer_score:max`` aggregate aliased ``orders.customer_score_max`` and surface its type under the bare name."""
        storage = YAMLStorage(base_dir=tempfile.mkdtemp())
        await storage.save_datasource(DatasourceConfig(name="test", type="postgres"))
        await storage.save_model(SlayerModel(
            name="customers", sql_table="customers", data_source="test",
        ...
        # Real attribute, not an auto-Mock: it feeds the cache key's credential digest (DEV-1755), which hashes it.
        mock_ds.credentials_json = None
        with patch.object(engine, "_resolve_datasource", new_callable=AsyncMock, return_value=mock_ds):
...
# DEV-1337: per-alias allowlists for "natively supports single-arg log10/log2" rendering. Outside these sets the current 2-arg LOG(base, x) form is kept. Mirrored in slayer/sql/generator.py — keep in sync.
...
class TestLogAliasPreservation:
    """DEV-1337 — user-written ``log10(x)`` / ``log2(x)`` must round-trip verbatim in emitted SQL on dialects that natively support those single-arg aliases. sqlglot's default behaviour normalises both into a generic ``Log(this=Literal(base), expression=arg)`` AST node and re-emits as ``LOG(base, x)`` for almost every dialect, which makes generated SQL diverge from the recipe formula text and (on dialects that lack 2-arg ``LOG``) can break a previously working call."""
    ...
    def log_model(self) -> SlayerModel:
        return SlayerModel(
            ...
            columns=[
                ...
                Column(name="status", sql="status", type=DataType.TEXT),
                Column(name="amount", sql="amount", type=DataType.DOUBLE),
                # Scalar log expressions inside Column.sql — the primary path the issue surfaced through.
                Column(name="log_amount", sql="log10(amount)", type=DataType.DOUBLE),
                Column(name="log2_amount", sql="log2(amount)", type=DataType.DOUBLE),
                # Negative-control: a non-alias literal base. Must keep the standard 2-arg LOG(base, x) form post-fix.
                Column(name="log3_amount", sql="log(3, amount)", type=DataType.DOUBLE),
                # ln(...) is a separate AST node (exp.Ln); the rewrite must not touch it.
    ...
    async def test_log10_in_column_sql_is_preserved(
        ...
        if dialect in _LOG10_NATIVE_DIALECTS:
            ...
            # Fallback: current 2-arg LOG behaviour is preserved on dialects without native single-arg log10 (oracle).
    ...
    async def test_log10_inside_filtered_column_survives_reparse(
        ...
        """A filtered Column (``Column.filter="..."``) wraps the resolved value in ``CASE WHEN ... THEN ... END`` and re-parses through sqlglot. The log-alias rewrite must survive that round-trip — a re-parse of ``LOG10(amount)`` would otherwise canonicalise back to a generic ``Log`` node and re-emit as ``LOG(10, amount)``."""
        model = SlayerModel(
            name="orders",
            sql_table="public.orders",
            data_source="test",
    ...
    async def test_log10_in_arithmetic_measure_is_preserved(
        ...
        """Arithmetic that mixes a log10 column-derived measure with COUNT(*). Pins that the rewrite survives the arithmetic enrichment path."""
        gen = SQLGenerator(dialect=dialect)
        query = SlayerQuery(
            source_model="orders",
    ...
    async def test_log_with_non_alias_base_unchanged(
        ...
        """Negative test: ``log(3, amount)`` (literal base ≠ 10/2) must keep the standard 2-arg form. The rewrite is scoped to bases 10 and 2 only."""
        gen = SQLGenerator(dialect=dialect)
        query = SlayerQuery(
            source_model="orders",
    ...
    async def test_ln_unchanged(
        ...
        """``ln(x)`` lives under a separate sqlglot AST node (``exp.Ln``); the rewrite must not affect it."""
        gen = SQLGenerator(dialect=dialect)
        query = SlayerQuery(
            source_model="orders",
        ...
        sql = await _generate(generator=gen, query=query, model=log_model)
        # T-SQL has no LN — sqlglot transpiles to LOG(x). Every other dialect keeps LN(...). We only assert the rewrite did not invent something.
...
class TestWindowFunctionInFilter:
    """DEV-1369 (reverses DEV-1336): a query filter that references a ``Column`` whose ``sql`` contains a window function (``OVER (...)``) no longer auto-promotes to a post-aggregation outer WHERE. The rank-family transforms (``rank`` / ``percent_rank`` / ``dense_rank`` / ``ntile``) cover top-N filtering in pure DSL, so the escape hatch is redundant. The engine raises a clear error directing the user to those transforms or to a multi-stage source_queries model."""

    async def test_filter_on_windowed_column_raises(
        self, generator: SQLGenerator, planets_model: SlayerModel,
    ) -> None:
        """Filtering on a Column whose sql contains a window function raises with an actionable message at enrichment time."""
        query = SlayerQuery(
    ...
    async def test_select_only_on_windowed_column_unchanged(
        ...
        """A windowed Column.sql is still legal as a *projection* — only as a filter target does it now error."""
        query = SlayerQuery(
            source_model="planets",
            dimensions=["name", "rn"],
        )
...
class TestCastEmissionColumn:
    """``Column.type`` declares the result type of the column expression; the generator wraps non-bare ``Column.sql`` in ``CAST(... AS <type>)``. Bare identifiers and ``sql=None`` paths are NOT wrapped (trust DB schema + sqlglot). ``DataType.TEXT`` is a no-op wrapper (cosmetic)."""
    ...
    async def test_bare_identifier_not_wrapped(self) -> None:
        """Bare ``sql='amount'`` (and ``sql=None``) trust the DB schema and sqlglot — no CAST emitted regardless of declared type."""
        model = SlayerModel(
            name="items",
            sql_table="public.items",
            data_source="test",
    ...
    async def test_idempotent_cast(self) -> None:
        """If the user pre-wrapped ``Column.sql`` in a CAST to the same target, the generator does NOT double-wrap."""
        model = SlayerModel(
            name="items",
            sql_table="public.items",
            data_source="test",
        ...
        query = SlayerQuery(source_model="items", dimensions=[ColumnRef(name="x")])
        sql = await _generate(gen, query, model)
        # Exactly one CAST in the projection. Any double-wrap would produce CAST(CAST(... AS ...) AS ...) — assert that pattern is absent.
        assert "CAST(CAST(" not in sql.upper()


class TestCastEmissionOpaqueType:
    """``CAST(x AS UNKNOWN)`` is not valid SQL in any dialect, so opaque types are skipped by ``_wrap_cast_for_type`` exactly like TEXT/None."""
    ...
    async def test_opaque_column_rejected_as_dimension(self) -> None:
        """An opaque column has no equality operator, so grouping by it would emit SQL the database refuses. Reject it up front with an actionable message rather than letting a raw driver error surface."""
        model = SlayerModel(
            name="places",
    ...
    async def test_opaque_column_emits_no_cast_when_projected(self) -> None:
        """The CAST guard still holds for a query that doesn't group by the opaque column: no ``CAST(... AS UNKNOWN)`` may reach the SQL."""
        model = SlayerModel(
            name="places",
            sql_table="public.places",
            data_source="test",
        ...
        sql = await _generate(gen, query, model)
        # No opaque cast reaches the SQL. CAST(COUNT(*) AS INT) is still legitimate, so assert the absence of UNKNOWN entirely rather than banning CAST(.
        assert "UNKNOWN" not in sql.upper()

    async def test_opaque_column_allowed_as_raw_row_projection(self) -> None:
        """Raw-row mode (``distinct_dimension_values=False``, no measures, DEV-1543) emits no top-level GROUP BY, so projecting an opaque column as a "dimension" is legal — only grouping is refused. Regression guard: the opaque-dimension check must NOT fire when the query doesn't group."""
        model = SlayerModel(
            name="places",
            sql_table="public.places",
            data_source="test",
...
class TestCastEmissionMeasure:
    """``ModelMeasure.type`` (when set) wraps the aggregation expression in a final CAST. ``None`` (default) → no cast."""
    ...
    async def test_measure_type_none_count_preserves_native_range(self, orders_model) -> None:
        ...
        sql = await _generate(generator=gen, query=query, model=orders_model)
        # INT remains the inferred metadata type, but must not narrow the backend's native count result unless the measure explicitly asks.
...
class TestCastEmissionNonBasePaths:
    """DEV-1361 follow-up: ``ModelMeasure.type`` and ``Column.type`` must wrap aggregation expressions in CAST across every emission path — not just the base ``_generate_base()`` path. Covers windowed CTEs, isolated filtered measure CTEs, percentile/median/stat-agg/weighted-avg builders."""
    ...
    async def test_windowed_sum_with_measure_type_wraps_in_cast(
        ...
        """Windowed sum CTE was previously emitting ``SUM(_src._w_value) AS alias`` with no CAST when the inline measure declared ``type=DataType.DOUBLE``."""
        gen = SQLGenerator(dialect="postgres")
        query = SlayerQuery(
        ...
        # The windowed CTE itself must contain the CAST around SUM(_src._w_value). DEV-1835: unified ``_cm_`` prefix, named after the canonical aggregate.
    ...
    async def test_windowed_sum_no_measure_type_casts_inferred_type(
        ...
        """DEV-1714 decision: with no EXPLICIT measure type, the windowed aggregation still CASTs on the type inferred from the source column (``revenue`` is ``DOUBLE``) — exactly as a plain aggregate does in the base path (DEV-1361). A windowed sum and a plain sum of the same no-type measure must emit identical CAST behavior; the legacy explicit-only distinction no longer exists on the typed pipeline."""
        gen = SQLGenerator(dialect="postgres")
        query = SlayerQuery(
    ...
    async def test_percentile_uses_column_type_for_inner_cast(self) -> None:
        """``_resolve_value_sql`` must propagate ``column_type`` so that non-bare ``Column.sql`` (e.g. ``json_extract(...)``) feeding percentile gets the inner pre-aggregation CAST applied."""
        model = SlayerModel(
            name="events",
    ...
    async def test_weighted_avg_uses_column_type_for_inner_cast(self) -> None:
        """``weighted_avg`` goes through ``_build_formula_agg`` → ``_resolve_value_sql``. With ``column_type`` propagation, non-bare Column.sql gets CAST'd inside the formula expansion."""
        model = SlayerModel(
            name="events",
            sql_table="public.events",
            data_source="test",
...
class TestStringHygieneDialectTranslation:
    """DEV-1378: lowercase string-hygiene operators are pass-through to the emitted SQL string, then re-parsed by sqlglot under the target dialect at WHERE-assembly. sqlglot's per-dialect emitter chooses each dialect's preferred spelling. These tests pin the actual emitted SQL across SQLite / Postgres / MySQL / DuckDB / ClickHouse so a future sqlglot upgrade that changes the spelling is caught."""
    ...
    @pytest.mark.parametrize(
        ...
        [
            # Dialects whose sqlglot emitter prefers the operator now render ||: the ScalarCall policy builds a typed exp.Concat. On Postgres this is also semantic — CONCAT() ignores NULLs, || propagates them.
...
def _build_score_model_dev1539(*, name: str = "m", score_sql: str) -> SlayerModel:
    """DEV-1539 test helper: build a minimal model with four numeric columns (``a, b, c, d``) and a derived ``score`` column whose ``sql`` is the multi-term expression under test. Used by both the local- branch positive test and the SQLite integration test so the model setup boilerplate isn't duplicated across files (Sonar ``new_duplicated_lines_density``)."""
    return SlayerModel(
        name=name,
...
def _build_backslash_risk_model_dev1539(*, name: str = "m") -> tuple[SlayerModel, str]:
    """DEV-1539 test helper: build a model with a ``risk`` column whose ``sql`` contains a literal double-backslash inside a string literal. Returns ``(model, double_backslash_literal)``. Used by both the WHERE-side and HAVING-side backslash-safety tests."""
    backslash = chr(92)
...
async def _build_joined_customers_orders_engine_dev1539(
    *, tmp_path, customers_score_sql: str,
    ...
    """DEV-1539 test helper: spin up a storage-backed engine with an ``orders`` model joined to a ``customers`` model whose ``score`` / derived column carries ``customers_score_sql``. Used by the dotted joined-column wrap test and the dotted-branch backslash-safety test so the storage + join boilerplate isn't duplicated."""
    storage = YAMLStorage(base_dir=str(tmp_path))
    await storage.save_datasource(DatasourceConfig(name="test", type="postgres"))
    await storage.save_model(SlayerModel(
        name="customers",
...
class TestFilterOuterParenWrapDev1539:
    """DEV-1539: defensive outer-paren wrapping at every site where a multi-term expression gets plopped into a filter context."""

    async def test_filter_inlines_multiterm_local_column_with_outer_parens(
        self, generator: SQLGenerator,
    ) -> None:
        """A query filter on a Column whose ``sql`` is a multi-term arithmetic expression must surface the inlined body wrapped in outer parens — ``(a + b) > 7``, not ``a + b > 7``."""
        ...
        sql = await _generate(generator, query, model)
        norm = _norm(sql)
        # The multi-term LHS of > 7 must be enclosed for precedence; the pipeline encloses the derived expression in its type CAST (CAST(m.a * 0.4 + ... AS DOUBLE PRECISION) > 7). Bounded quantifier [^)]+ avoids S5852 backtracking.
        ...
        # All four weighted terms must survive inside the enclosed LHS — the single-quantifier regex alone would still match if a later term were dropped (CodeRabbit). Plain substring checks keep the S5852-safe shape.
        for _term in ("m.a", "m.b", "m.c", "m.d"):
            assert _term in m.group(0), (
                f"Expected weighted term {_term} inside the enclosed LHS; "
                f"got: {m.group(0)}"
            )
        # Negative: the trailing arithmetic term must NOT land bare next to ``> 7`` (the un-enclosed, precedence-broken shape).
    ...
    async def test_filter_inlines_bare_column_no_parens(
        ...
        """When a Column is a bare identifier (``sql == name`` or sql is a single identifier), no extra parens are added around the qualified reference. ``WHERE orders.customer_id > 100``, not ``WHERE (orders.customer_id) > 100``."""
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
            filters=["customer_id > 100"],
    ...
    async def test_filter_inlines_multiterm_joined_column_with_outer_parens(
        ...
        """The dotted joined-column filter branch must also wrap inlined non-bare Column.sql bodies in outer parens. Filter ``joined_model.score > 7`` where the joined column's sql is a multi-term arithmetic expression."""
        ...
        norm = _norm(sql)
        # The inlined joined-column expression must be enclosed before > 7, same as the local-column sibling, via its type CAST. Bounded quantifier [^)]+ keeps the S5852-safe shape.
        assert "WHERE" in norm
        where_clause = norm.split("WHERE", 1)[1]
    ...
    async def test_dsl_compare_lhs_binop_wrapped(
        ...
        """A DSL filter ``a + b > 7`` must emit ``(a + b) > 7`` so the precedence of the multi-term arithmetic LHS is explicit by inspection, not only by SQL operator-precedence rules."""
        # Use two bare-name columns so the LHS is a Compare(BinOp(...), 7) and the BinOp wrap applies at the DSL layer, not at column inlining (both columns inline as bare qualified identifiers).
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
    ...
    async def test_dsl_compare_bare_lhs_not_wrapped(
        ...
        """A DSL filter with a bare Name LHS (``x > 7``) must not gain spurious parens: ``WHERE orders.x > 7``, not ``WHERE (orders.x) > 7``."""
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
    ...
    async def test_dsl_compare_call_lhs_not_wrapped(
        ...
        """A DSL filter whose LHS is a function call (``lower(status) == 'a'``) must not gain spurious parens around the call. Only ``BinOp`` and ``BoolOp`` LHSes get wrapped."""
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
            filters=["lower(status) == 'active'"],
    ...
    async def test_dsl_compare_lhs_boolop_wrapped(
        ...
        """A DSL filter whose LHS is an ``ast.BoolOp`` (rare but valid) — e.g. ``(a and b) > 7`` — must emit ``(a AND b) > 7``. Covers the BoolOp half of the Site 2 wrap rule symmetrically with the BinOp half."""
        # NB: customer_id and id are bare-identifier columns, so the only wrap on emit is the DSL-level BoolOp wrap under test.
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
    ...
    async def test_dsl_compare_rhs_boolop_wrapped(
        ...
        """The RHS-side counterpart: ``x == (a or b)`` must emit ``x = (a OR b)``."""
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
    ...
    async def test_filter_inline_preserves_backslash_in_joined_column_sql(
        ...
        """Site 1b backslash safety: the dotted joined-column inlining branch must also use lambda-replacement in ``re.sub`` so backslashes inside the joined column's SQL aren't silently halved when substituted into the filter text."""
        # customers.risk must be multi-term (to fire the non-bare inlining branch) and contain a backslash literal; we rename the shared helper's score column to a backslash-bearing expression.
        backslash = chr(92)
        ...
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
            # The joined column is named ``score`` in the helper; that's the alias under which the backslash-bearing sql lives.
    ...
    async def test_having_substitution_preserves_backslash_in_agg_sql(
        ...
        """Site 3 backslash safety: HAVING measure-substitution must use lambda-replacement in ``re.sub`` so backslashes inside the aggregated value's source SQL aren't silently halved when the emitted ``agg_sql`` is substituted into the HAVING text."""
    ...
    async def test_having_multiterm_measure_wrapped(
        ...
        """When a HAVING filter references a measure whose aggregation expression is a multi-term form (e.g., ``SUM(x * w) / SUM(w)`` for a ``weighted_avg`` measure), the substituted expression in the emitted HAVING must be wrapped in outer parens."""
        model = SlayerModel(
            name="sales",
            sql_table="public.sales",
            data_source="test",
        ...
        # The substituted SUM(...) / NULLIF(SUM(...)) must be parenthesised before > 0. A (SUM( substring check false-passes (it matches inside NULLIF(SUM(), so anchor on the comparator and check the whole LHS is wrapped.
        ...
        # The HAVING expression must START with an open paren — the outer wrap. (After ``strip()`` above any pretty-print indentation is gone.)
    ...
    async def test_having_simple_measure_not_wrapped(
        ...
        """A HAVING filter on a simple single-aggregation measure (``revenue:sum > 0``) must NOT gain spurious outer parens: ``HAVING SUM(...) > 0``, not ``HAVING (SUM(...)) > 0``. Only multi-term ``Binary``/``Connector`` ``agg_expr`` shapes warrant the wrap."""
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
            measures=[ModelMeasure(formula="revenue:sum")],
    ...
    async def test_filter_inline_preserves_backslash_in_column_sql(
        ...
        """``re.sub(pattern, repl, source)`` interprets backslashes in the replacement string as escape sequences — a literal ``\\\\`` in the inlined Column.sql gets silently halved to ``\\`` in the emitted WHERE. Using ``lambda _: repl`` for the replacement side-steps the bug. This test pins the fix."""
        model, double_bs = _build_backslash_risk_model_dev1539()
        query = SlayerQuery(
            source_model="m",
            measures=[ModelMeasure(formula="*:count")],
        ...
        # Must not raise (e.g., ``re.error: bad escape \\b``) and must preserve the doubled backslash in the emitted SQL.
        sql = await _generate(generator, query, model)
        # The original SQL literal had two backslashes; after substitution via lambda replacement, both must survive.
        assert f"'{double_bs}'" in sql, (
    ...
    @pytest.mark.parametrize(
        ["body_sql", "connector"],
        [
            # sqlglot 30.4.3: ``exp.And`` is a subclass of ``exp.Func``, so a pure inverse-atomic check would mis-classify this as atomic and skip the wrap. The compound check must fire first.
    ...
    async def test_filter_inlines_and_or_connector_column_with_outer_parens(
        ...
        """Column.sql whose root is ``a AND b`` / ``a OR b`` (sqlglot ``exp.And`` / ``exp.Or``) must be wrapped on inline. These inherit from ``exp.Func`` in sqlglot 30.4.3, so the inverse-atomic check at ``_filter_inline_needs_paren_wrap`` would skip them — the compound-types check has to fire first."""
        model = SlayerModel(
            name="m",
            sql_table="public.m",
            data_source="test",
        ...
        sql = await _generate(generator, query, model)
        norm = _norm(sql)
        # The pipeline encloses the connector-rooted expression in its type CAST (CAST(m.archived AND m.deleted AS BOOLEAN)) so IS NULL binds to the whole connector.
        m = _re.search(
        ...
        # And the wrap really protects the IS NULL precedence — the char before `IS NULL` must be `)`.
    ...
    async def test_filter_inlines_not_predicate_column_with_outer_parens(
        ...
        """A Column.sql whose root is ``NOT <expr>`` (sqlglot ``exp.Not``) must be wrapped on inline so a surrounding predicate like ``IS NULL`` binds to the whole expression, not just the inner operand. Without the wrap, ``NOT archived IS NULL`` reads as ``NOT (archived IS NULL)`` — different semantics from the intended ``(NOT archived) IS NULL``. The original ``_filter_inline_needs_paren_wrap`` only matched ``Binary``/ ``Connector`` and missed this shape."""
        model = SlayerModel(
            name="m",
            sql_table="public.m",
            data_source="test",
        ...
        sql = await _generate(generator, query, model)
        norm = _norm(sql)
        # The typed pipeline encloses the NOT-rooted derived expression in its type CAST: ``CAST(NOT m.archived AS BOOLEAN) IS NULL`` — the NOT binds inside the CAST, so ``IS NULL`` applies to the whole predicate.
        m = _re.search(
    ...
    async def test_filter_inlines_between_predicate_column_with_outer_parens(
        ...
        """A Column.sql whose root is ``BETWEEN`` (sqlglot ``exp.Between``) must be wrapped on inline. ``exp.Between`` is NOT ``exp.Binary`` — the original positive check missed it."""
        model = SlayerModel(
            name="m",
            sql_table="public.m",
            data_source="test",
    ...
    async def test_filter_inlines_in_predicate_column_with_outer_parens(
        ...
        """A Column.sql whose root is ``IN`` (sqlglot ``exp.In``) must also be wrapped — same gap as ``Not`` / ``Between`` in the original positive check."""
        model = SlayerModel(
            name="m",
            sql_table="public.m",
            data_source="test",
    ...
    def test_dotted_path_substitution_does_not_match_longer_path(self) -> None:
        """Site 1b (dotted joined-column branch in ``slayer/engine/enrichment.py``) must guard against substituting a shorter dotted col_name (e.g. ``customers.score``) inside a longer dotted reference (e.g. ``customers.score.extra``). Without the trailing ``(?!\\.)`` lookahead — which the local and HAVING branches both have — a 2-hop col_name mis-substitutes as a prefix of a 3-hop ref, mangling the emitted SQL."""
        ...
        # Only the standalone 2-hop ref is rewritten; the 3-hop ref is left intact for its own (later) substitution.
    ...
    def test_having_substitution_does_not_match_dotted_continuation(
        ...
        """The HAVING-side measure substitution regex in ``_build_where_and_having`` must guard against matching a measure name when it appears as the prefix of a dotted continuation. Without ``(?!\\.)`` after ``\\b``, a measure named ``foo`` mis-substitutes inside a literal ``foo.bar`` in the filter SQL."""
        # A bare foo.bar can't be synthesised in a HAVING string via normal DSL channels (dotted refs parse as Attributes); real risk for multi-stage paths, so test the regex directly.
        col_name = "foo"
        agg_sql = "SUM(amount)"
...
class TestBigQueryAliasMangling:
    """BigQuery rejects column names containing dots — SLayer's universal ``<model>.<column>`` alias convention has to be mangled to ``___`` on the way out (and reversed on the way back at the engine)."""
    ...
    async def test_no_dotted_aliases_in_bigquery_sql(self, orders_model: SlayerModel) -> None:
        ...
        sql = await _generate(gen, query, orders_model)
        # Every backtick-quoted column alias must not contain a dot (BigQuery rejects it). Scope to AS `...` and ORDER/GROUP BY so dotted table FQ refs (backticked segments) don't trip the check.
        ALIAS_PATTERNS = [
    ...
    async def test_other_dialects_keep_dotted_aliases(self, orders_model: SlayerModel) -> None:
        # Mangling is bigquery-only; postgres / sqlite / etc. must keep the dotted alias form (which clients and ORDER BY resolvers depend on).
        gen = SQLGenerator(dialect="postgres")
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
    ...
    async def test_inner_stage_render_also_mangles_aliases(
        ...
        """Inner-stage renders (``source_queries`` stages behind a query-backed model) must mangle aliases too — otherwise the outer stage's references to inner-stage columns would mismatch what the inner stage actually emits."""
        ...
        # The inner stage keeps every alias (so the outer stage can reach them); all aliases must be mangled.
        ...
        # Inner emit and outer reference agree on the SAME ``___`` alias: each appears at least twice (emitted once, referenced once).
    ...
    async def test_rewrite_mangles_the_final_public_projection(
        ...
        """On BigQuery, the FINAL emitted SQL's public projection carries ``___``-mangled aliases — the rewrite ran on SQL whose projection had already been decided."""
        query = SlayerQuery(
            source_model="orders",
            measures=[ModelMeasure(formula="*:count")],
            dimensions=[ColumnRef(name="status")],
        ...
        # The hidden hoisted column was narrowed away by the projection (so the projection genuinely ran) yet is itself mangled where it does appear (so the rewrite covered the inner layers too).
...
class TestCrossModelAggregateSourceSqlJoinInference:
    """DEV-1526 harvest (Stage 1): a CROSS-MODEL aggregate whose TARGET column's ``Column.sql`` crosses a FURTHER join must pull the implied LEFT JOINs into the per-plan ``_cm_*`` CTE body. The fix lands Stage 4 (Law-1 inside the CTE ScopeFrame); the discovery tests are pinned strict-xfail until then. ``test_bare_target_column_no_spurious_join_in_cte`` stays green (a bare cross-model aggregate already works). Chain: ``orders_x → customers_v2 → regions → countries``."""
    ...
    async def test_first_last_source_join_in_ranked_subquery(self, storage) -> None:
        engine = await self._engine_with(storage, self._customers_v2(extra_columns=[
            Column(name="deep_pop", sql="regions.population", type=DataType.DOUBLE),
            # The ranking column must be one of the TARGET's own — the ranked CTE is rooted there and a host column is not in its scope.
            Column(name="signup_at", sql="signup_at", type=DataType.TIMESTAMP),
        ]))
        query = SlayerQuery(
            source_model="orders_x",
    ...
    async def test_bare_target_column_no_spurious_join_in_cte(self, storage) -> None:
        """Green: a bare cross-model aggregate already works — the ``_cm_*`` CTE has no spurious deeper join."""
        engine = await self._engine_with(storage, self._customers_v2())
        query = SlayerQuery(
            source_model="orders_x",
...
class TestWindowedMeasureGuards:
    """DEV-1496 harvest (Stage 1): windowed measures must emit a ``_wm_`` range-join CTE and RAISE loudly on shapes the primitive does not support, rather than silently degrade to a plain grouped aggregate. Today the typed pipeline drops ``window=`` silently, so every guard is pinned strict-xfail to Stage 10 (DEV-1714). The composite/hidden/mixed guards are DEV-1504."""
    ...
    async def test_windowed_with_transform_raises(self, orders_model: SlayerModel) -> None:
        ...
        # DEV-1835: a windowed measure coexisting with a transform sibling now renders; the SQL must be scope-closed with __regroup__ lowered away.
    ...
    async def test_windowed_as_transform_input_raises(self, orders_model: SlayerModel) -> None:
        ...
        # DEV-1835 lift: a windowed aggregate feeding a transform now renders instead of raising; the emitted SQL must be scope-closed and fully lowered (no ``__regroup__`` sentinel).
        sql = await _engine_generate(query=query, model=orders_model)
        assert_scope_closed(sql, dialect="postgres")
        assert "__regroup__" not in sql
    ...
    async def test_windowed_in_arithmetic_raises(self, orders_model: SlayerModel) -> None:
        ...
        # DEV-1835 lift: a windowed aggregate inside scalar arithmetic now renders instead of raising; scope-closed and fully lowered.
        sql = await _engine_generate(query=query, model=orders_model)
        assert_scope_closed(sql, dialect="postgres")
        assert "__regroup__" not in sql
    ...
    async def test_windowed_filter_only_hidden_raises(self, orders_model: SlayerModel) -> None:
        ...
        # DEV-1835 lift: a filter-only (hidden) windowed aggregate now renders instead of raising; scope-closed and fully lowered.
        sql = await _engine_generate(query=query, model=orders_model)
        assert_scope_closed(sql, dialect="postgres")
        assert "__regroup__" not in sql
    ...
    async def test_windowed_mixed_aggregate_filter_raises(self, orders_model: SlayerModel) -> None:
        ...
        # DEV-1835 lift: a filter mixing a windowed and a plain aggregate now renders instead of raising; scope-closed and fully lowered.
        sql = await _engine_generate(query=query, model=orders_model)
        assert_scope_closed(sql, dialect="postgres")
        assert "__regroup__" not in sql
    ...
    async def test_windowed_malformed_duration_raises(
        ...
        """A malformed compact duration must raise a clear ``ValueError`` at plan time, not silently degrade."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
    ...
    async def test_windowed_non_string_duration_raises(self, orders_model: SlayerModel) -> None:
        """Codex C4: a non-string ``window`` value (e.g. ``window=90``) must be normalized to a ``ValueError`` — never a raw ``TypeError`` from feeding a number to the compact-duration regex."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
    ...
    async def test_windowed_transform_input_precedence_not_selected(
        ...
        """Codex C1: a windowed aggregate nested in a transform interns as a HIDDEN dependency slot, so the transform guard (G4) must win over the hidden-slot guard (G6) — the error says ``transform``, never ``selected``."""
        orders_model.default_time_dimension = "created_at"
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
        ...
        # DEV-1835: the G4/G6 guards are gone — a windowed aggregate nested in a transform now renders; the SQL must be scope-closed and fully lowered.
        sql = await _engine_generate(query=query, model=orders_model)
        assert_scope_closed(sql, dialect="postgres")
        assert "__regroup__" not in sql
    ...
    def test_windowed_guards_fire_at_plan_time(self, case: str) -> None:  # NOSONAR(S3776) — flat case dispatch for a parametrized guard matrix; each branch builds one (query, bundle, exc, match) tuple, kept inline so the guard scenario reads next to its expectation.
        """Decision D-a / Codex C1+C2: every windowed guard raises in the PLANNER (``plan_query``) — before any SQL is rendered. Proves the guards are plan-time (a renderer-only guard would fail this by not raising here)."""
    ...
    async def test_windowed_default_time_dimension_not_selected_raises(self) -> None:
        """CR#3: a windowed measure whose only time dimension is the model's ``default_time_dimension`` (never selected as a query ``time_dimensions`` entry, so never interned as a bucket-grain slot) must raise the clean G2 message — not crash on the required ``window_time_dimension_slot_id``."""
        model = SlayerModel(
            name="orders", sql_table="public.orders", data_source="test",
            columns=[
                Column(name="id", sql="id", type=DataType.DOUBLE, primary_key=True),
    ...
    async def test_windowed_mixed_row_filter_raises(self, orders_model: SlayerModel) -> None:
        """Codex round 5: a single filter mixing a windowed measure with a ROW predicate (not just a plain aggregate — G7's case) is reclassified whole to POST, so the row part would neither filter pre-aggregation nor resolve against an unprojected ``_base`` column. Reject it at plan time."""
        query = SlayerQuery(
            source_model="orders",
            time_dimensions=[TimeDimension(dimension=ColumnRef(name="created_at"), granularity=TimeGranularity.MONTH)],
            measures=[{"formula": "revenue:sum(window='90d')", "name": "rev_w"}],
        ...
        # DEV-1835 lift: a filter mixing a windowed aggregate with a row predicate now renders instead of raising; scope-closed and fully lowered.
        sql = await _engine_generate(query=query, model=orders_model)
        assert_scope_closed(sql, dialect="postgres")
        assert "__regroup__" not in sql

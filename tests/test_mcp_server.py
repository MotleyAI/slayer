...
class TestModelsSummary:
    def test_empty(self, mcp_server) -> None:
        result = _call(mcp_server, "models_summary")
        assert "No models defined" in result
        assert "create_model" in result
...
class TestAddMeasures:
    def test_add(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(
            name="orders", sql_table="t", data_source="test",
            measures=[Measure(name="count", type=DataType.COUNT)],
        ))
        result = _call(mcp_server, "add_measures", {
            "model_name": "orders",
            "measures": [
                {"name": "total", "sql": "amount", "type": "sum", "description": "Total amount"},
            ],
        })
        parsed = json.loads(result)
        ...
        assert model.measures[1].name == "total"

    def test_duplicate_name(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(
            name="orders", sql_table="t", data_source="test",
            measures=[Measure(name="count", type=DataType.COUNT)],
        ))
        result = _call(mcp_server, "add_measures", {
            "model_name": "orders",
            "measures": [{"name": "count", "sql": "x", "type": "sum"}],
        })
        assert "already exists" in result

    def test_invalid_type(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(name="orders", sql_table="t", data_source="test"))
        result = _call(mcp_server, "add_measures", {
            "model_name": "orders",
            "measures": [{"name": "bad", "sql": "x", "type": "invalid"}],
        })
    ...
    def test_model_not_found(self, mcp_server) -> None:
        result = _call(mcp_server, "add_measures", {
            "model_name": "nope",
            "measures": [{"name": "x", "sql": "y", "type": "sum"}],
        })
        assert "not found" in result
...
class TestAddDimensions:
    def test_add(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(name="orders", sql_table="t", data_source="test"))
        result = _call(mcp_server, "add_dimensions", {
            "model_name": "orders",
            "dimensions": [
    ...
    def test_duplicate_name(self, mcp_server, storage: YAMLStorage) -> None:
        ...
        result = _call(mcp_server, "add_dimensions", {
            "model_name": "orders",
            "dimensions": [{"name": "status", "sql": "x", "type": "string"}],
        })
        assert "already exists" in result
    ...
    def test_invalid_type(self, mcp_server, storage: YAMLStorage) -> None:
        ...
        result = _call(mcp_server, "add_dimensions", {
            "model_name": "orders",
            "dimensions": [{"name": "bad", "sql": "x", "type": "invalid"}],
        })
        assert "Invalid dimension type" in result


class TestDeleteMeasuresDimensions:
    def test_delete_measure(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(
            name="orders", sql_table="t", data_source="test",
            measures=[Measure(name="count", type=DataType.COUNT), Measure(name="total", sql="x", type=DataType.SUM)],
        ))
        result = _call(mcp_server, "delete_measures_dimensions", {
            "model_name": "orders",
            "names": ["total"],
        })
        parsed = json.loads(result)
        assert parsed["deleted"][0] == {"name": "total", "kind": "measure"}
        model = storage.get_model("orders")
        assert len(model.measures) == 1

    def test_delete_dimension(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(
            name="orders", sql_table="t", data_source="test",
            dimensions=[Dimension(name="id", type=DataType.NUMBER), Dimension(name="status", type=DataType.STRING)],
        ))
        result = _call(mcp_server, "delete_measures_dimensions", {
            "model_name": "orders",
            "names": ["status"],
        ...
        parsed = json.loads(result)
        assert parsed["deleted"][0] == {"name": "status", "kind": "dimension"}

    def test_not_found(self, mcp_server, storage: YAMLStorage) -> None:
        storage.save_model(SlayerModel(name="orders", sql_table="t", data_source="test"))
        result = _call(mcp_server, "delete_measures_dimensions", {
            "model_name": "orders",
            "names": ["nonexistent"],

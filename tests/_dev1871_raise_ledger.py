...
ROWS: Tuple[LedgerRow, ...] = (
    ...
    _row(_EE, "check_windowed_time_dimension", "ValueError",
         ...
         "checker", "time-axis", True, "checker"),
    _row(_EE, "_check_shift_family_key", "ValueError",
    ...
    _row(_EE, "_check_shift_family_key", "ValueError",
         "'…' does not support a row-level (non-aggregate) leaf inside a composite or nested-transform input; every leaf must be an aggregate. Compute the row-level value in an earlier stage of a multi-stage `source_queries` model and reference its aggregate here.",
         "checker", "positions", True, "checker"),
    _row(_EE, "check_non_shift_transform_row_leaf", "ValueError",
         "Transform '…' cannot consume the row-level (non-aggregate) leaf '…', which refines the query grain: it would inflate the base grain to one row per (bucket, …-value). Aggregate the leaf — e.g. …(…:sum) — or project '…' as a query dimension.",
         "checker", "positions", True, "checker"),
    ...
    # Out-of-scope population conjunct (OR/NOT mix, or several branches) with a
    # plain aggregate inline over the population: fail closed rather than fan.
    _row(_EE, "check_population_filter_in_pushdown_scope", "ValueError",
         "Filter … is outside semi-join pushdown scope (…); with a plain aggregate computed inline over the population, applying it through the join would multiply the population's rows. Split the filter's conjuncts onto one branch each, or restate it so its cross-path references share one branch.",
         "checker", "filters", True, "checker"),
    # An unanalyzable filter dependency is unsafe, not local — host and producers
    # alike, in every mode, whether or not the query aggregates.
    _row(_EE, "check_filter_dependencies_analyzable", "ValueError",
         "Filter … depends on derived column …, whose definition no supported dialect can analyse for join dependencies; an unanalyzable dependency is unsafe. Fix the column's SQL, or remove the filter.",
         "checker", "filters", True, "checker"),
    ...
    _row(_EE, "check_computed_dimension", "ValueError",
         "The transform '…' inside computed dimension … must take an aggregate input — a transform acts on aggregates, e.g. …(amount:sum(partition_by=city)).",
         "checker", "regroup-roots", True, "checker"),
    _row(_EE, "check_computed_dimension", "ValueError",

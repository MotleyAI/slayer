...
def test_postgres_falls_back_to_exact_count_distinct() -> None:
    out = PostgresDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
...
def test_sqlite_falls_back_to_exact_count_distinct() -> None:
    out = SqliteDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
...
def test_mysql_falls_back_to_exact_count_distinct() -> None:
    out = MysqlDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
...
def test_duckdb_native_approx_count_distinct() -> None:
    out = DuckdbDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
...
def test_clickhouse_native_uniq() -> None:
    out = ClickhouseDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
...
def test_bigquery_native_approx_count_distinct() -> None:
    out = BigqueryDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="bigquery").upper()
    assert "APPROX_COUNT_DISTINCT(" in sql


def test_snowflake_native_approx_count_distinct() -> None:
    out = SnowflakeDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="snowflake").upper()
    assert "APPROX_COUNT_DISTINCT(" in sql


def test_tsql_native_approx_count_distinct() -> None:
    out = TsqlDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="tsql").upper()
    assert "APPROX_COUNT_DISTINCT(" in sql


def test_redshift_native_approximate_count_distinct() -> None:
    out = RedshiftDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
...
def test_trino_native_approx_distinct() -> None:
    out = TrinoDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="trino").lower()
    assert "approx_distinct(" in sql


def test_presto_native_approx_distinct() -> None:
    out = PrestoDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="presto").lower()
    assert "approx_distinct(" in sql


def test_databricks_native_approx_count_distinct() -> None:
    out = DatabricksDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="databricks").lower()
    assert "approx_count_distinct(" in sql


def test_spark_native_approx_count_distinct() -> None:
    out = SparkDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))
    sql = out.sql(dialect="spark").lower()
    assert "approx_count_distinct(" in sql


def test_oracle_native_approx_count_distinct() -> None:
    out = OracleDialect().build_approx_count_distinct(col_expr=exp.column("customer_id"))

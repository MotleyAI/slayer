...
The completed table for an ORDER BY ref matching no declared/public slot:
  ...
  local row column, grouped| ValueError                           (DEV-1712)
  joined row column        | UnresolvableOrderColumnError         (DEV-1712)
...
class TestOrderOnlyTransform:
    ...
    async def test_order_only_transform_in_raw_rows_mode(self, engine) -> None:
        """``distinct_dimension_values=False`` with an order-only transform.

        The typed pipeline deliberately does NOT enforce DEV-1543's
        "no measure references in order" rule — that check lives in
        ``enrichment.py``, which the typed ``_prepare_pipeline`` never calls,
        and ``test_ungrouped_input_local_aggregate_order_still_works`` already
        pins the permissive behaviour for a bare aggregate. Transforms follow
        the same rule: the inner aggregate induces the GROUP BY.
        """
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
...
class TestStillRejected:
    async def test_grouped_row_column_order_still_raises(self, engine) -> None:
        query = SlayerQuery(
            source_model="orders",
            dimensions=[ColumnRef(name="status")],
            measures=[ModelMeasure(formula="*:count")],
        ...
        with pytest.raises(ValueError) as ei:
            await _sql(engine, query)
        assert "GROUP BY" in str(ei.value), ei.value

...
hop pushes as a correlated semi-join on the host base query (each population row
once), and every producer rooted at the population inherits that one disposition
— partitioned, windowed, first/last, nested, broadcast-local and association.
Same-branch conjuncts a consumer's grain materialises bind inline to one related
row; out-of-scope and unanalyzable conjuncts fail closed.

Covers ``queries/semantics`` (Filters restrict by association or fail loudly;
Grain guarantee) and ``queries/cross-model-aggregates`` (Producer filter
routing). Executed-value oracles live in ``tests/_dev1900_fixtures.py``.
"""
...
def _slayer_warnings(caught) -> list:
    """Captured warnings whose class is defined in the ``slayer`` package — any
    SLayer warning, current or future, past incidental library warnings."""
...
class TestProducersInheritDisposition:
    ...
    async def test_first_last_producer_over_time_axis(self, backend):
        """spend:last is fan-insensitive so the values were already right; the
        push is what removes the fanning join from the producer body."""
        dialect, engine = backend
    ...
    async def test_nested_producer_inherits(self, backend):
        """avg(sum(spend, partition_by=tier)) — the inner totals count each
        customer once; the semi-join lives in the host-rooted producer body."""
        dialect, engine = backend
    ...
    async def test_host_target_nesting_body(self, backend):
        """A host-rooted producer nesting a target-rooted one carries the
        population semi-join in its own (outer) body."""
        dialect, engine = backend
    ...
    async def test_count_over_population(self, backend):
        """*:count of the population per tier — one plain aggregate over the
        restricted rows, each customer once."""
        _, engine = backend
    ...
    async def test_association_producer_same_branch(self, backend):
        """C17 — the amount-in-(20,30) rows bind per status: c1's 'new' order and
        c2's 'ok' order land in their own cells (never both in both)."""
        _, engine = backend
...
class TestSameRowBinding:
    ...
    async def test_single_same_branch_filter_binds_one_cell(self, backend, mode):
        """S8 — the only amount-20 order is c1's 'new'; the status dimension binds
        the filter to that same row, so one cell survives (associate and the
        broadcast-local producer alike)."""
        _, engine = backend
        resp = await engine.execute(cust_q(
            measures=[SPEND], dimensions=["orders.status"],
    ...
    async def test_two_ok_orders_same_status_cell(self, backend):
        """Decision 5 — the status dimension materialises the fanning branch, so
        the inline spend counts c1 once despite its two 'ok' orders (420, not 520)."""
        _, engine = backend
        resp = await engine.execute(cust_q(
    ...
    async def test_mixed_conjunct_splits_inline_and_pushed(self, backend):
        """One filter string, one inline conjunct (tier='gold') and one pushed
        (orders.status='ok'): gold customers with an ok order."""
        dialect, engine = backend
    ...
    async def test_two_branches_restrict_independently(self, backend):
        """S9 — an ok order AND a region event of value>=50 (only North), each
        branch satisfied independently, each customer once."""
        dialect, engine = backend
...
class TestRawRowsAndDimensionsOnly:
    ...
    async def test_out_of_scope_conjunct_without_aggregate_applies(self, backend):
        """S12 — an OR-mixed conjunct with no inline aggregate stays applied to
        the result rows through the join."""
        _, engine = backend
        resp = await engine.execute(cust_q(
            dimensions=["tier"],
    ...
    async def test_out_of_scope_conjunct_dropped_from_producer(self, backend):
        """S12 — an OR-mixed conjunct a producer cannot push is dropped from it
        with the dropped-filter warning, while the host base keeps applying it."""
        _, engine = backend
        resp = await engine.execute(cust_q(
    ...
    async def test_fanning_axis_window_keeps_frame_rows(self, backend):
        """C18 — a date_range on the window's own fanning time axis bounds the
        visible buckets without truncating the window's earlier frame rows."""
        _, engine = backend
...
class TestProducerOnlySpine:
    async def test_producer_only_value_and_empty(self, backend):
        """C19 — one row with the producer's value; a predicate no order passes
        yields zero rows via the empty-base spine, never one NULL row."""
        _, engine = backend
...
class TestFailClosed:
    ...
    async def test_unanalyzable_filter_fails_closed(self, unparse_backend, mode):
        """S10 — an unparseable derived dependency fails closed in every mode,
        with or without a measure, naming the filter and the column."""
    ...
    async def test_out_of_scope_conjunct_with_aggregate_fails_closed(self, backend, mode):
        """S11 — an OR-mixed conjunct with a plain aggregate inline over the
        population fails closed, naming the filter, the reason and the remedy."""
        _, engine = backend
        with pytest.raises(ValueError) as ei:
    ...
    async def test_ambiguous_host_correlation_fails_closed(self, parallel_backend, mode):
        """C8 — the population reaches the filtered model only across a pair of
        parallel edges; fail closed naming the candidates, never guess a hop."""

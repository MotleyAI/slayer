...
Same harness and blessing protocol as ``tests/test_dev1745_golden_sql.py``; only
the matrix differs, covering every ORDER BY render path, the host-grain wrap, the
re-rooted cross-model CTE, and both assembled transform chains across dialects.
"""
...
#: DuckDB for a third null-ordering regime; BigQuery and T-SQL mangle dotted aliases at emission.
DIALECTS = ["postgres", "sqlite", "duckdb", "tsql", "bigquery"]

# ``<case_id>::<dialect>`` -> why this entry may change now; a committed state keeps it empty.
...
def _cases() -> dict:
    ...
    return {
        ...
        # A windowed measure with a well-formed date_range mints a BETWEEN filter; pin it byte-identical.
        ...
        # Both push into the producer by semi-join now; ids kept stable ("unreachable" is historical).
        ...
        # C13: one transform key under TWO names — the step CTE surfaces both, not re-emit the first.
        ...
        # consecutive_periods (own cp_reset/cp_value CTE path) under TWO names surfaces both.
        ...
        # Two aggregates of one column in the base CTE surface in plan order, not hash-seed order.
        ...
        # --- dependency-split step-CTE shapes: a window over a window -> step1 then step2 ---
...
async def _generate_one(query: SlayerQuery, dialect: str):
    """Emitted SQL, or a structured record keeping the raised error's COMPLETE message."""
...
def test_ordering_cases_actually_emit_an_order_by(baseline) -> None:
    """Vacuity guard: every ``order/`` entry must still emit an ORDER BY."""
...
def test_reroot_cases_actually_reroot(baseline) -> None:
    """Vacuity guard: a ``_cm_`` alias must be present, else the shape stopped re-rooting."""
    for key, value in baseline.items():
...
#: Step-CTE structure each chain case emits. ``step2`` marks a dependency split (a second batch);
#: ``consecutive_periods`` layers its own ``cp_`` CTEs, not ``step<n>``.
...
def test_chain_cases_emit_expected_step_ctes(baseline) -> None:
    """Vacuity guard: pin ``step1`` / ``step2`` presence so a dropped or collapsed batch fails loudly."""
    seen = set()
    for key, value in baseline.items():

...
class SlayerResponse:
    ...
    def __init__(self, data: List[Dict[str, Any]], columns: Optional[List[str]] = None, sql: Optional[str] = None):
        self.data = data
...
class SlayerQueryEngine:
    ...
    def execute(self, query: SlayerQuery) -> SlayerResponse:
        # Preprocessing: resolve variables, snap to whole periods
        if query.variables:
            query = query.resolve_variables()
        ...
        return SlayerResponse(data=rows, sql=sql)
    ...
    def _enrich(
        ...
        # Resolution chain: query time_dimensions (if exactly 1) → model default.
        ...
        def _add_transform(name: str, transform: str, measure_alias: str,
            ...
            enriched_transforms.append(EnrichedTransform(
                ...
                alias=alias, offset=abs(offset), granularity=granularity,

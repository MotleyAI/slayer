...
def create_mcp_server(storage: StorageBackend):
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "Typical workflow: models_summary → inspect_model → query. "
            "To connect a new database: create_datasource → describe_datasource (to verify) → ingest_datasource_models → models_summary."
        ),
    )
    ...
    def query(
        ...
        Args:
            model: Name of the model to query (from models_summary).
            fields: Data columns to return. Each is a formula: {"formula": "count"} (measure),
                {"formula": "revenue / count", "name": "aov"} (arithmetic),
                {"formula": "cumsum(revenue)"} (cumulative sum), {"formula": "change(revenue)"} (period diff),
                {"formula": "change_pct(revenue)"} (% change), {"formula": "lag(revenue, 2)"} (N periods ago),
                {"formula": "last(revenue)"} (most recent value), {"formula": "time_shift(revenue, -1, 'year')"} (YoY),
            ...
            filters: Filter conditions as formula strings. Examples: "status == 'completed'",
                ...
                "contains(name, 'acme')", "having(count > 10)".
            ...
            variables: Template variables for filter placeholders, e.g. {"status": "active"}.
    ...
    def models_summary() -> str:
        """List all available models with their schemas (dimensions, measures). Does not include sample data — use inspect_model for that."""
        names = storage.list_models()
        if not names:
            return "No models defined. Use create_model to define one manually, or create_datasource + ingest_datasource_models to auto-generate from a database."
        ...
        return json.dumps({"models": models, "count": len(models)}, indent=2, default=str)
    ...
    def add_measures(
        ...
        """Add one or more custom measures with SQL expressions to an existing model.

        Args:
            model_name: Name of the model to add measures to.
            measures: List of measure specifications. Each: {"name": "total", "sql": "amount", "type": "sum", "description": "Total amount"}.
                Types: count, count_distinct, sum, avg, min, max.
        """
        model = storage.get_model(model_name)
        if model is None:
        ...
        for spec in measures:
            ...
            item = {"name": name, "type": measure_type, "sql": spec.get("sql")}
        ...
        return json.dumps({
            ...
            "count": len(added),
            "message": f"Added {len(added)} measure(s) to '{model_name}'",
        }, indent=2)
    ...
    def add_dimensions(
        ...
        """Add one or more custom dimensions with SQL expressions to an existing model.

        Args:
            model_name: Name of the model to add dimensions to.
            dimensions: List of dimension specifications. Each: {"name": "region", "sql": "region", "type": "string", "description": "Customer region"}.
                Types: string, number, time, date, boolean.
        """
        model = storage.get_model(model_name)
        if model is None:
        ...
        existing_names = {d.name for d in model.dimensions}
        added = []
        ...
        for spec in dimensions:
            ...
            existing_names.add(name)
            item = {"name": name, "type": dim_type, "sql": spec.get("sql")}
            if spec.get("description"):
                item["description"] = spec["description"]
            added.append(item)
        ...
        return json.dumps({
            ...
            "count": len(added),
            "message": f"Added {len(added)} dimension(s) to '{model_name}'",
        }, indent=2)

    @mcp.tool()
    def delete_measures_dimensions(
        model_name: str,
        names: List[str],
    ) -> str:
        """Delete one or more measures or dimensions by name from a model.

        Args:
            model_name: Name of the model.
            names: Names of the measures or dimensions to delete.
        """
        model = storage.get_model(model_name)
        if model is None:
            return f"Model '{model_name}' not found."
        ...
        for name in names:
            ...
            if match:
                ...
                deleted.append({"name": name, "kind": "measure"})
                continue
            ...
            if match:
                ...
                deleted.append({"name": name, "kind": "dimension"})
                continue
        ...
        return json.dumps({
            ...
            "deleted": deleted,
            "count": len(deleted),
            "message": f"Deleted {len(deleted)} item(s) from '{model_name}'",
        }, indent=2)
    ...
    def update_model(
        ...
        """Update a model's metadata (description, data source) without replacing its full definition.

"""SQLite-based storage for models, named queries, and datasources."""
...
class SQLiteStorage(StorageBackend):
    ...
    def _persist_model_sync(self, model: SlayerModel) -> None:
    ...
    def _persist_query_sync(self, query: NamedQuery) -> None:
        data = json.dumps(query.model_dump(mode="json", exclude_none=True))
        with sqlite3.connect(self.db_path) as conn:
            conn.execute("INSERT OR REPLACE INTO queries (name, data) VALUES (?, ?)", (query.name, data))

    def _get_query_sync(self, name: str) -> Optional[str]:
        with sqlite3.connect(self.db_path) as conn:
            row = conn.execute("SELECT data FROM queries WHERE name = ?", (name,)).fetchone()
        return row[0] if row else None

    def _list_queries_sync(self) -> List[str]:
        with sqlite3.connect(self.db_path) as conn:
            rows = conn.execute("SELECT name FROM queries ORDER BY name").fetchall()
        return [r[0] for r in rows]

    def _delete_query_sync(self, name: str) -> bool:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("DELETE FROM queries WHERE name = ?", (name,))
            return cursor.rowcount > 0
    ...
    async def _persist_model(self, model: SlayerModel) -> None:
        await asyncio.to_thread(self._persist_model_sync, model)
    ...
    async def _persist_query(self, query: NamedQuery) -> None:
        await asyncio.to_thread(self._persist_query_sync, query)

    async def get_query(self, name: str) -> Optional[NamedQuery]:
        raw = await asyncio.to_thread(self._get_query_sync, name)
        return NamedQuery.model_validate(json.loads(raw)) if raw else None

    async def list_queries(self) -> List[str]:
        return await asyncio.to_thread(self._list_queries_sync)

    async def delete_query(self, name: str) -> bool:
        return await asyncio.to_thread(self._delete_query_sync, name)

...
class SQLGenerator:
    ...
    def _generate_with_computed(self, enriched: EnrichedQuery, base_sql: str) -> str:
        ...
        # Separate time_shift transforms (handled via self-join, not stacked CTEs)
        ...
        for t in time_shifts:
            ...
            interval_sql = self._build_interval_sql(
                offset=t.offset, granularity=t.granularity or "month",
            )
            from_clause += (
                f"\nLEFT JOIN {shift_name}"
                f"\n    ON {final_cte}.{time_col} = {shift_name}.{time_col} + {interval_sql}"
            )
    ...
    def _build_transform_sql(t) -> str:
        ...
        if t.transform == "cumsum":
            ...
            return f"LAG({measure}, {t.offset}) OVER ({order_clause})"
            ...
            # Handled separately via self-join CTE in _generate_with_computed
            raise ValueError("time_shift should not reach _build_transform_sql")
        else:
            raise ValueError(f"Unsupported transform: {t.transform}")

    def _build_interval_sql(self, offset: int, granularity: str) -> str:
        """Build a SQL interval expression for time_shift."""
        if self.dialect == "sqlite":
            # SQLite doesn't have INTERVAL — handled differently
            # For SQLite, use date arithmetic in the JOIN condition

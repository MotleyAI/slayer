...
Formulas are parsed using Python's `ast` module (see `slayer/core/formula.py`). Available functions: `cumsum`, `lag`, `lead`, `change`, `change_pct`, `rank`, `last` (most recent value via `FIRST_VALUE`), `time_shift` (year-over-year etc. via self-join CTE). Formulas support arbitrary nesting — e.g., `change(cumsum(revenue))` or `cumsum(revenue) / count`.
...
**Variables**: use `{var_name}` placeholders resolved from `variables`:
```json
{
  "model": "orders",
  "fields": [{"formula": "count"}, {"formula": "revenue_sum"}],

...
projected dimension or time dimension on the same join branch) applies to that consumer
on the same related row instead, so filtering and grouping on one branch bind to one row.
...
warning (an error under `to_many_handling: "error"`), and when a plain aggregate is
evaluated inline over the population rows the query SHALL fail at plan time with a typed
error naming the filter, the reason, and the remedy — never a silently multiplied
aggregate.
...
- **WHEN** the same filter runs with `dimensions: ["tier"]` and no measures, or with only
  aggregates that compute in producers

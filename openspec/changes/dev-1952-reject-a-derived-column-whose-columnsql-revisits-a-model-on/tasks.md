...
- [x] 2.3 `slayer/engine/compile/stages.py`: `_canonical_path` → `tuple(path)`; `_reverse_hops` sets `fwd = None` in the `except` so its `_PushBlocked` raise stays one site; verify `tests/test_dev1871_raise_parity.py` (ledger unchanged) and `tests/test_dev1909_population_pushdown.py` pass.
- [x] 2.4 `slayer/sql/column_expansion.py`: `resolve_ref_target`, `_lenient_path` catch → `None` (~~`resolve_default_qualifier_path`~~ superseded by DEV-1908's cancelling door, merged 2026-09-22); verify the definition-default regression tests (scalar and expression default in the DEV-1900 back-hop shape; a revisiting default probed from the owner frame cancels, never raises) and `tests/test_dev1931_default_home.py`, `tests/test_dev1892_parameter_typing.py` unchanged.
...
- [x] 6.1 Apply the approved Axiom 1 clause and `[enforced: test:tests/test_dev1952_derived_revisit.py]` tag in `architecture/semantics.arc42.md`; verify `poetry run python tools/arch_check.py` and `npx -y likec4@1.47.0 validate architecture` pass.
- [x] 6.2 Append the one approved sentence to the derived-columns paragraph in `docs/concepts/models.md`; verify no new page (nav in `zensical.toml` unchanged).

## 7. Tests

- [x] 7.1 Write `tests/test_dev1952_derived_revisit.py` (unit only; sqlite + duckdb params for the query-time block) covering every scenario in the delta spec plus the regression guards (via-host reroot; definition defaults; ambiguity; `_hop_walk_reason`/`terminal_model` unchanged); verify each new pin fails without the fix by a revert check.
- [x] 7.2 Consent-covered edits: flip `test_revisit_is_guarded` to `pytest.raises(CircularJoinPathError)`; add both classes to `tests/test_errors_hierarchy.py`; verify both files pass.

## 8. Gate

- [x] 8.1 `openspec validate dev-1952-reject-a-derived-column-whose-columnsql-revisits-a-model-on --strict` passes.
- [x] 8.2 `poetry run pytest -m "not integration"`, `poetry run ruff check slayer/ tests/`, `poetry run python tools/arch_check.py` pass and `poetry run basedpyright` shows no new errors vs baseline.

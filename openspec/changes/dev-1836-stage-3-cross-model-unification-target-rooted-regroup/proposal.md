...
- **Migration**: `_cm_` forward/re-rooted/filtered-local, cross-model `_rk_`, and cross-model partitioned aggregates become target-rooted regroup producers; `cross_model_planner.py`, `CrossModelAggregatePlan`, `_narrow_shared_grain_to_partition`, the cross-model exclusion in `combined_partitioned_aggregates`, and the `classify_isolation` dispatch are deleted. A post-discovery total-routing invariant (every aggregate leaf: inline | producer | explicit rejection) lands before the deletion.

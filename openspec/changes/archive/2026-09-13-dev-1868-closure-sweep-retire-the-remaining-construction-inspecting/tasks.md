...
- [x] 1.1 Guard-bypassed characterization tests for W1 (cross-model first/last ×
  ...
  stage.
- [x] 1.2 Characterize W2 and W3 the same way (transform-nested cross-model partitioned;
...
- [x] 2.1 New executed-value tests, SQLite + DuckDB, hand-computed oracles on the
...
- [x] 2.2 Series-shift executed tests `tests/test_dev1868_series_shift_exec.py`:
  ...
  then passing.
- [x] 2.3 Plan-structure tests (F5/F6): producer count / substitution ownership / attach
...
- [x] 2.4 Residue negative tests (F7): transform-without-aggregate, ungrained-aggregate,
  ...
  verified by pytest.
- [x] 2.5 Law-pool extension (D8): add cross-model partitioned operands to
...
- [x] 2.6 Compile-level dialect coverage (F8): generated-SQL tests for the new shapes
...
- [x] 3.1 W1: remove `check_partitioned_measures` raise 1; fix only the
  characterization-documented failing stage (if any); verified by 2.1 green.
- [x] 3.2 W2: remove raise 2; wire the inner cross-model partitioned aggregate's desugar
  to `_synthesize_cross_model_producer` so the transform consumes the placeholder (D2);
  verified by 2.1/2.3 green.
- [x] 3.3 W3: desugar cross-model composite operands to placeholders before phase
  ...
  verified by 2.1/2.3 green.
- [x] 3.4 Series-shift emission mode (D4): materialised-series branch in
  ...
  by 2.2 green.
- [x] 3.5 Residue split + P9 relocation (D6, D7): two `ValueError`s in
  ...
  raise-parity scan.
- [x] 3.6 Mechanics (W8): remove 4 `DEFERRAL_SITES` entries; `guards.baseline` 5 → 1;
...
- [x] 3.7 Full unit suite + lint green (`poetry run pytest -m "not integration"`,
...
- [x] 4.1 Docs: one-sentence updates in `docs/concepts/formulas.md` / `queries.md` where
  the lifted restrictions are stated; verified by grep for stale restriction wording.
- [x] 4.2 arc42 (user-approval-gated, exact diff presented first): axiom 9 tag
...
- [x] 4.3 Linear: amend DEV-1873 residue list (transform-in-dimension split rule); comment

...
- [x] 1.1 Add `to_many_handling` to `SlayerQuery` (Literal, default `"broadcast"`),
...
- [x] 1.2 Storage migration `SlayerQuery` v3→v4 (`strict: true` → `"error"`, else
  dropped) with `CURRENT_VERSIONS` bump; verify via migration tests loading v3
  payloads in all three strict states.
- [x] 1.3 Swap the parameter in REST `QueryRequest` and the MCP `query` tool
...
- [x] 1.4 Thread the mode through `PreboundQuery` and `StrictQueryCarrier` into every
...
- [x] 2.1 Extend attribution classification to local aggregates with unattributable
...
- [x] 2.2 Replace the strict gate with the `error`-mode gate covering broadcasts
...
- [x] 2.3 Fix the broadcast reason for reachable-but-fanning dimensions (names the
  hop classification, not "unreachable"); verify via warning-payload tests.

## 3. Association producer

- [x] 3.1 Add the `association` `ProducerKernel` variant (entity-key columns +
...
- [x] 3.2 Emit the two-level GROUP BY in the generator (one picked column per input
...
- [x] 3.3 Route filters into the association producer via the existing producer
...
- [x] 3.4 Typed errors for `window=`/`first`/`last` combinations and missing root
  unique key; verify via error tests and an unchanged guard-ratchet baseline.
- [x] 3.5 Warnings: `associated` payload kind (surfaced twice, suppressed for explicit
...
- [x] 4.1 Associate-mode executed-value suite on SQLite + DuckDB: cross-model,
...
- [x] 4.2 Regression tests for the multiplication finding (duplicate same-status
  orders: default broadcasts — never 350; associate returns 250/100/60); verify they
  fail on the pre-change engine.
- [x] 4.3 Rewrite the dev-1853 pinned local-shape test on duplicate-carrying data
  asserting both modes (approved); verify the dev-1853 suite passes.
- [x] 4.4 `tests/test_law_dice_slice.py` in the generative law harness (filter `d=v`
  ≡ slice cell `v` under associate); verify across the harness shape family on
  SQLite + DuckDB stride.
- [x] 4.5 Golden baselines: byte-identity of cross-model broadcast defaults; new
...
- [x] 5.1 `architecture/semantics.arc42.md`: reword axiom 8 (drop the override
...
- [x] 5.2 Update `docs/concepts/queries.md` (strict row → `to_many_handling`,
  ...
  docs/skills (grep).
- [x] 5.3 Divergence ledger (`divergences.md`) enumerating the local-shape value
  flip, strict rejection, new warning payloads, and re-blessed goldens; verify every
  behavior-flipping test cites its entry.

## 6. Gate

- [x] 6.1 Full non-integration suite green (`poetry run pytest -m "not

...
- [x] 1.1 `tests/_dev1908_fixtures.py`: DEV-1900 graph + `regions → countries` (to-one,
...
- [x] 1.2 `tests/test_dev1908_default_cancellation.py`: home paths via `Aggregate.home_path`
...
- [x] 1.3 `tests/test_dev1908_walker.py`: the cancelling walk — cancel at the root, at a
  ...
  on the current tree.
- [x] 1.4 `tests/test_dev1908_route.py`: `attributable_from_root`, `reroot_from_root` and
...
- [x] 1.5 `tests/test_dev1908_golden_sql.py` + `tests/golden/dev1908_sql_baseline.json`
...
- [x] 1.6 `tests/test_dev1900_home_path.py::TestReverseHopDefaultFailsClosed`: replace
...
- [x] 1.7 Run `poetry run pytest tests/test_dev1908_default_cancellation.py
  tests/test_dev1908_walker.py tests/test_dev1908_route.py -x` and confirm the expected
  failures.

## 2. Implementation (spec-implement stage)

- [x] 2.1 `slayer/core/join_walker.py`: add the cancelling walk (D1, D2, D4) — resolve
...
- [x] 2.2 `slayer/sql/column_expansion.py`: `resolve_default_qualifier_path` takes the
...
- [x] 2.3 `slayer/engine/reference_closure.py`: `_frame_dotted_key` consumes the new
...
- [x] 2.4 `slayer/engine/home.py`: `_default_param_keys` / `_default_home_candidate_paths`
...
- [x] 2.5 `slayer/engine/join_safety.py`: the route primitive (D9) and its use in the
...
- [x] 2.6 `slayer/engine/compile/stages.py` + `slayer/ir/planned.py`: the re-aggregation
...
- [x] 2.7 `slayer/sql/generator.py`: `_default_frag_entry` per D7 (owner-forward raw at
...
- [x] 2.8 Bless `tests/golden/dev1908_sql_baseline.json` per the divergence protocol
  (1.5). Verify: golden test green on all seven dialects.
- [x] 2.9 Run the full non-integration suite (`poetry run pytest -m "not integration"`) and
...
- [x] 3.1 `architecture/semantics.arc42.md`: Axiom 2.4 → "…defaults resolved as
  references from the owning model with **cancellation**: a qualifier naming a dataset
  already on its path reads that row (`a.b.a ≡ a`) rather than re-joining, any other
  unreachable qualifier anchored at the query root. Only definition defaults cancel — a
  revisiting query path is refused, a revisiting `Column.sql` likewise `[target:
  DEV-1952]`."; Axiom 2.6 → "…is resolved there, a qualifier naming a dataset on the
  anchor's path cancelling back to it, any other qualifier the anchor cannot reach
  ...
  unchanged (concise wording approved 2026-09-21, James's revision). Verify:
  `poetry run python tools/arch_check.py` green.

## 4. Docs

- [x] 4.1 `docs/concepts/models.md` (Custom aggregations): one sentence — parameter
...
- [x] 5.1 `poetry run ruff check slayer/ tests/` clean.
- [x] 5.2 `poetry run basedpyright` — no new errors vs baseline; shrink in touched files.
- [x] 5.3 `poetry run python tools/arch_check.py` green.
- [x] 5.4 Codex full-diff review clean (last local gate before push) — two findings
  verified as plan-consistent (edge-onto-visited miss per D2; token-prefix route is
  conservative/fail-closed for divergent spellings, no wrong value), no fix warranted.

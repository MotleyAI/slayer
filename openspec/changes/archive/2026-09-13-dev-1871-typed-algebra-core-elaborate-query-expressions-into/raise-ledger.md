...
| elaborate_env.py | check_raw_rows_filter_measure_ref | DistinctDimensionValuesError | checker | positions | yes | checker | 1 |  | distinct_dimension_values=False rejects measure references, but filter … contains one. … |
| elaborate_env.py | check_raw_rows_order_measure_ref | DistinctDimensionValuesError | checker | positions | yes | checker | 1 |  | distinct_dimension_values=False rejects measure references, but order item … contains one. … |
| elaborate_env.py | check_raw_rows_order_measure_ref | DistinctDimensionValuesError | checker | positions | yes | checker | 1 |  | distinct_dimension_values=False rejects measure references, but order item … resolves to a saved measure on... |
| elaborate_env.py | check_raw_rows_order_measure_ref | DistinctDimensionValuesError | checker | positions | yes | checker | 1 |  | distinct_dimension_values=False rejects measure references, but order item … resolves to a saved measure. … |
...
| elaborate_env.py | check_reserved_regroup_prefix | ValueError | checker | regroup-roots | yes | checker | 1 |  | Column(s) … use the reserved '__regroup__' prefix, which collides with the regroup primitive's placeholders... |
| elaborate_env.py | check_raw_rows_no_aggregate_slots | DistinctDimensionValuesError | checker | positions | yes | checker | 1 |  | distinct_dimension_values=False rejects measure references, but this query references the aggregation … in ... |
| elaborate_env.py | check_order_target_has_slot | PositionTypingError | checker | positions | yes | checker | 1 |  | ORDER BY expression is not supported: … has no materialisable slot. Order by an aggregate, a transform, a c... |
...
| elaborate_env.py | check_computed_dim_name_collision | ValueError | checker | names | yes | checker | 1 |  | Computed dimension name … collides with an existing column or measure on model …. Choose a different name. |
| elaborate_env.py | check_computed_dim_name_collision | ValueError | checker | names | yes | checker | 1 |  | Computed dimension name … collides with a query measure of the same name. Choose a different name. |
...
| elaborate_env.py | check_stage_flatten_collision | ValueError | checker | names | yes | checker | 1 |  |  |
| elaborate_env.py | check_measure_dedupe_collision | ValueError | checker | names | yes | checker | 1 |  | Measures … and … both derive the result key … but compute different values; rename one (set 'name') to disa... |
| elaborate_env.py | check_measure_dedupe_collision | ValueError | checker | names | yes | checker | 1 |  | Measures … and … merge into one result column … but declare different label/type; rename one (set 'name') t... |
...
| elaborate_env.py | validate_model_filter | ValueError | checker | model-filters | yes | checker | 1 |  | Model filter … references measure …. Model filters can only reference table columns (WHERE). Use query-leve... |
| elaborate_env.py | validate_model_filter | ValueError | checker | model-filters | yes | checker | 1 |  | Model filter … references column … whose SQL contains a window function. Factor it into a multi-stage sourc... |
| bind_inputs.py | _resolve_main_time_dimension | AmbiguousReferenceError | bind | time-axis | yes | bind | 1 |  |  |
| bind_inputs.py | _resolve_main_time_dimension | UnknownReferenceError | bind | time-axis | yes | bind | 1 |  |  |
| elaborate_env.py | type_position_conjunct | PositionTypingError | checker | positions | yes | checker | 1 |  | This … expression is valid as neither a field nor a measure. Field typing failed: it references …, availabl... |
| elaborate_env.py | type_position_conjunct | PositionTypingError | checker | positions | yes | checker | 1 |  | This … expression references …, so it is not a field, and measure typing is unavailable because the query h... |
| elaborate_env.py | check_measure_name_collision | MeasureNameCollidesWithColumnError | checker | names | yes | checker | 1 |  |  |
| elaborate_env.py | check_canonical_alias_shadows_column | CanonicalAliasShadowsColumnError | checker | names | yes | checker | 1 |  |  |
| elaborate_env.py | check_duplicate_measure_name | DuplicateMeasureNameError | checker | names | yes | checker | 1 |  |  |

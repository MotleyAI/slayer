...
9. **Temporal-cast suppression is a dialect capability applied at every declared-cast seam.** `SqlDialect.declared_cast_type(dt)` returns `dt`; the SQLite dialect returns `None` for DATE / TIMESTAMP. The generator routes each declared-cast target through it: `_slot_cast_type` (the single funnel every slot cast site reads) applies it to its result, and the two seams that don't read `_slot_cast_type` — `_resolve_sql` and `_derived_column_expr` — pass their type through it before `_wrap_cast_for_type` (Codex finding: the derived-column seam bypassed the other two). `_ranked_value_cast_type` and `_filter_cast_type` stay: they encode redundancy on every dialect, not a dialect quirk. Alternative (suppress temporal casts globally) rejected: it changes five dialects' SQL and undoes the deliberate Postgres `date_trunc` overload cast.

...
- [ ] 1.1 `queries/multi-stage` execution tests (SQLite + DuckDB, hand-computed values): the `[x, c(src x), b(joins c), root]` list in every non-root permutation; a stage reaching a sibling through an inline model's joins and through nested `source_queries` two levels down; one stage reading three siblings; a stored join to model `customers` beside a sibling named `customers` resolving to the model. Verify: red today (the chain shape fails with `target 'c' not in source bundle`).
- [ ] 1.2 `plan_stages` tests: returned order equals `topologically_order_stages`; `stage_reads` correct (root included, plan-ordered) for every shape in 1.1 plus a producer rooted at a sibling and a semi-join (`EXISTS`) hop over a sibling; `[named, unnamed_root]` accepted; unnamed non-root, self-reference, root-referenced and cycle rejected. Verify: red today (no `stage_reads`, source-only sort).
- [ ] 1.3 Declared-edge tests on captured `assemble_with_chain` entries: each stage relation declares its hoisted CTEs, its reuse deps and its `stage_reads`; every hoisted CTE and every root entry declares its statement's `stage_reads`; a later stage body reusing an earlier stage's shared producer declares that producer. Verify: red today.
- [ ] 1.4 Precondition test: `generate_planned_stages` fed a planned list with a stage before a sibling it reads (root last) raises `ValueError` naming both. Verify: red today (currently emits SQL).

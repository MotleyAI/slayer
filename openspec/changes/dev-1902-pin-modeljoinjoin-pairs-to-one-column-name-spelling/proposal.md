...
- Importers emit logical names: Cube keeps member names and drops a join whose ON operand is a non-base member or names no member (report warning); dbt synthesises a hidden base column for a foreign entity no dimension covers and skips a non-bare `expr` (warning); OSI checks both sides against the contract before appending a relationship; ingestion applies its one column rename to `join_pairs` too.

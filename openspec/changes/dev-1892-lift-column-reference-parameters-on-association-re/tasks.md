...
- [ ] 1.1 `tests/test_dev1892_parameter_typing.py` — executed values on SQLite + DuckDB:
...
- [ ] 1.2 `tests/test_dev1892_residue.py` — typed errors at plan time, ref-free (no
...
- [ ] 1.3 `tests/test_dev1892_plan_structure.py` — the kernel carries `picked_params`;
...
- [ ] 1.4 `tests/test_dev1892_fold_equivalence.py` — the folded pick path keeps executed
  ...
  verified by pytest.
- [ ] 1.5 Determination pair (spec: semantics › nested-path seeds): outer dimension
...
- [ ] 1.6 `tests/test_dev1892_golden_sql.py` + `tests/golden/dev1892_sql_baseline.json`
  across the seven Tier-1 dialects for the lifted shapes; verified by the golden harness.
- [ ] 1.7 Re-point approved existing tests: `test_dev1841_association_errors.py::
...
- [ ] 2.1 `slayer/core/keys.py`: `_AggregateArgValue` admits `AggregateKey`;
  `reaggregation_operand_keys` walks a re-aggregation root's args and kwargs; verified by
  1.3's bind-stage regression.
- [ ] 2.2 `slayer/engine/binding.py`: `_bind_agg_arg` binds an `AggCall` through `_bind`;
  positional values fold onto declared names as today; verified by 1.1 parity cases.
- [ ] 2.3 `slayer/engine/bind_inputs.py`: call `check_attached_param_requires_attached_source`
  on pre-lowering roots next to `check_time_shift_input`; verified by 1.2.

## 3. Checker

- [ ] 3.1 `slayer/engine/elaborate_env.py`: add `check_parameter_determined` and
...
- [ ] 4.1 `slayer/engine/join_safety.py`: `grain_determines` (grain member; aggregate
...
- [ ] 4.2 One parameter-resolution helper (explicit non-scalar args/kwargs +
...
- [ ] 4.3 `_synthesize_association_producer`: after input safety, resolve → check against
...
- [ ] 4.4 `_synthesize_reaggregation_producer`: resolve → check against the union grain →
...
- [ ] 5.1 `slayer/ir/planned.py`: `PickedParam` (Pydantic) and
  `AssociationProducerKernel.picked_params`; verified by 1.3.
- [ ] 5.2 `slayer/sql/generator.py`: one pick helper (type-branched: derived expansion /
...
- [ ] 6.1 `tests/_dev1871_raise_ledger.py`: two rows out, two in; verified by the parity
  test.
- [ ] 6.2 Full golden suite: every moved key (DEV-1841 `assoc/*`, any other) enters its
...
- [ ] 6.3 `poetry run pytest -m "not integration"`, `poetry run ruff check slayer/ tests/`,
  `poetry run basedpyright` (no new errors), `poetry run python tools/arch_check.py` all
  green.

## 7. Docs, architecture, Linear

- [ ] 7.1 Docs, one sentence each: `docs/concepts/queries.md` associate-eligibility list
...
- [ ] 7.2 `architecture/semantics.arc42.md` axiom 2 `[review]` →

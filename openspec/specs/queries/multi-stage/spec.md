...
stages were supplied in. The last entry SHALL stay the root. Duplicate stage names,
an unnamed non-root stage, a
...
- **WHEN** an inline model's nested `source_queries` reference a sibling stage at any depth
- **THEN** that stage is ordered after the sibling

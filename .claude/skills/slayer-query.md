...
**Functions**: `contains(col, 'val')`, `starts_with(col, 'val')`, `ends_with(col, 'val')`, `between(col, 'a', 'b')`, `having(expr)`
...
The `fields` parameter specifies what data columns to return. Formulas are parsed by `slayer/core/formula.py`.
...
query = SlayerQuery(
    ...
    fields=[
        ...
        {"formula": "lag(revenue_sum, 2)", "name": "rev_2_ago"},
        {"formula": "rank(revenue_sum)"},
    ],
)
```

Available formula functions: `cumsum`, `lag`, `lead`, `change`, `change_pct`, `rank`, `last` (most recent value), `time_shift` (year-over-year etc. via self-join CTE).

Time dimension resolution for time-dependent functions: query `time_dimensions` (if exactly 1) -> model `default_time_dimension` -> error.

## Variables (Filter Templates)

Use `{var_name}` placeholders in filter strings, resolved from `variables` dict:

```python
filters=["status == '{status}'", "customer_id == '{cid}'"],
variables={"status": "completed", "cid": 42}
```

Single `{var}` preserves native type; mixed strings do substitution.

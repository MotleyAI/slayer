...
A cross-model measure is computed in a sub-query rooted at ITS OWN model and joined back, so a 1:N host join never multiplies it. It gets the exact per-group value for dimensions the engine can prove safe from that model (a primary key on the far side of each hop, or declared join `cardinality: many_to_one`/`one_to_one`); any other dimension gets the BROADCAST value (the safe-grain total repeated), reported in `.warnings` as `kind: "broadcast"` — declare the join cardinality to make it exact. A query filter reachable from the sub-query's root only across an unproven hop still restricts the measure — pushed down silently as a correlated EXISTS semi-join over the same bidirectional traversal as every other hop (no declared reverse join needed; each target row counted once if related to ≥1 passing row; ClickHouse needs server ≥ 25.4). A correlation hop spanned by two or more edges fails the query with the ambiguous-hop error in every mode; only a filter with no resolvable path (or mixing local and joined refs under OR/NOT) applies to local measures only and warns (`kind: "unreachable_filter_dropped"`). `to_many_handling` picks how a broadcast dimension resolves — `broadcast` (default; repeat and warn), `associate` (per-cell value over the distinct associated entities, warned `kind: "associated"` as cells overlap and are not additive), or `error` (refuse); a stored query's retired `strict: true` migrates to `error` (fresh input carrying `strict` is rejected), and pushed filters never error in any mode. A filter ON the cross-model value (`"customers.score:avg > 4"`) drops failing groups, uniformly with local aggregate filters. Cross-model aggregates also take `window=` (the query's active time dimension must be attributable from the measure's model), `partition_by=` (each explicit key must be attributable — else a hard error, except under `to_many_handling: "associate"`, which attributes an unattributable key per distinct entity), `first`/`last`, and work inside dimension expressions.

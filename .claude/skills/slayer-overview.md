...
Querying: `query` (`source_model` is optional — omit it and the population/root is inferred as the smallest model determining every queried dimension along provably to-one joins, reported as `population` / `population_inferred`), `recommend_root_model` (the explain surface for that rule: given `model.column` / `model.metric` items, recommends the root that determines every column to-one — measures/suffixed items attach reachably — plus each item's join path; optional `root_hint` forces an intended root when feasible; returns partial-root `coverage` when none determines all)
